...
\usepackage[keepaspectratio]{animate}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#blockA <- hist.blocks[[2]][[2]]}
\hlstd{blockA} \hlkwb{<-} \hlstd{hist.blocks[[}\hlnum{3}\hlstd{]][[}\hlstr{"y6010"}\hlstd{]]}
...
{\centering \includegraphics[width=0.8\textwidth]{figure/manual_blocking-2} 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#par(mfrow = c(4, 2))}
\hlcom{#for (i in c(2, 5, 20, 30, 65, 100, 190, 220))}
\hlcom{#for (i in c(2, 3, 4, 10, 30, 40, 50, 60))}
...
\end{verbatim}
\begin{alltt}
\hlcom{#unlink("figure/animated_block_history.gif")}
\hlcom{#system("convert -delay 0.5 FIGS/anim*.jpg figure/animated_block_history.gif")}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{getShapeDist} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{sched}\hlstd{,} \hlkwc{sh}\hlstd{) \{}
    ...
    \hlstd{d} \hlkwb{<-} \hlnum{0}
    \hlstd{J} \hlkwb{<-} \hlnum{1}
    \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{(}\hlkwd{nrow}\hlstd{(sched)}\hlopt{-}\hlnum{1}\hlstd{)) \{}
        \hlstd{p} \hlkwb{<-} \hlstd{sched[i,} \hlkwd{c}\hlstd{(}\hlstr{"stop_lon"}\hlstd{,} \hlstr{"stop_lat"}\hlstd{)]}
        \hlkwa{for} \hlstd{(j} \hlkwa{in} \hlstd{J}\hlopt{:}\hlkwd{nrow}\hlstd{(sh)) \{}
            \hlstd{q1} \hlkwb{<-} \hlstd{sh[j,} \hlkwd{c}\hlstd{(}\hlstr{"lon"}\hlstd{,} \hlstr{"lat"}\hlstd{)]}
            \hlstd{q2} \hlkwb{<-} \hlstd{sh[j}\hlopt{+}\hlnum{1}\hlstd{,} \hlkwd{c}\hlstd{(}\hlstr{"lon"}\hlstd{,} \hlstr{"lat"}\hlstd{)]}
...
\hlkwd{getBlock}\hlstd{(blockA}\hlopt{$}\hlstd{trip_id, blockA}\hlopt{$}\hlstd{trip_start_date[}\hlnum{1}\hlstd{], mbta)} \hlkwb{->} \hlstd{DD}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in rep(blockInfo\$trip\_id, sapply(blockShapes, nrow)): invalid 'times' argument}}\end{kframe}
\end{knitrout}
