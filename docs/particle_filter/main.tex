...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in sqliteSendQuery(con, statement, bind.data): error in statement: no such table: shapes}}\begin{alltt}
\hlkwd{plotShape}\hlstd{(shapeIDs[}\hlnum{1}\hlstd{,} \hlnum{1}\hlstd{])}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in dbGetQuery(con, sprintf("{}SELECT shape\_pt\_lon, shape\_pt\_lat FROM shapes WHERE shape\_id='\%s'"{}, : error in evaluating the argument 'statement' in selecting a method for function 'dbGetQuery': Error in sprintf("{}SELECT shape\_pt\_lon, shape\_pt\_lat FROM shapes WHERE shape\_id='\%s'"{},\ \ (from <text>\#3) : \\\#\#\ \  object 'shapeIDs' not found}}\end{kframe}
\end{knitrout}
