...
\hlstd{plotShape} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{id}\hlstd{) \{}
    \hlstd{sh} \hlkwb{<-} \hlkwd{dbGetQuery}\hlstd{(con,} \hlkwd{sprintf}\hlstd{(}\hlstr{"SELECT shape_pt_lon, shape_pt_lat FROM shapes WHERE shape_id='%s'"}\hlstd{, id))}
    \hlkwd{plot}\hlstd{(sh}\hlopt{$}\hlstd{shape_pt_lon, sh}\hlopt{$}\hlstd{shape_pt_lat,} \hlkwc{type} \hlstd{=} \hlstr{"l"}\hlstd{,} \hlkwc{asp} \hlstd{=} \hlnum{1}\hlstd{,}
...
which uses Earth's radius $R = 6371000$~m:
\begin{align*}
  a_i &= \left( r_1 - z_{1i} \right) \cos\left(\frac{r_2 + z_{2i}}{2}\right) \\
  b_i &= r_2 - z_{2i} \\
  e_i &= R \sqrt{a^2 + b^2},
\end{align*}
...
\hlstd{distanceFlat} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{y}\hlstd{,} \hlkwc{z}\hlstd{,} \hlkwc{R} \hlstd{=} \hlnum{6371000}\hlstd{) \{}
    ...
    \hlcom{#R * sqrt(((y[1] - z[1, ]) * cos((y[2] + z[2, ]) / 2))^2 + (y[2] - z[2, ])^2)}

    \hlstd{Y} \hlkwb{<-} \hlstd{y} \hlopt{*} \hlstd{pi} \hlopt{/} \hlnum{180}
    \hlstd{Z} \hlkwb{<-} \hlstd{z} \hlopt{*} \hlstd{pi} \hlopt{/} \hlnum{180}
    \hlstd{a} \hlkwb{<-} \hlstd{(Z[}\hlnum{1}\hlstd{,]} \hlopt{-} \hlstd{Y[}\hlnum{1}\hlstd{])} \hlopt{*} \hlkwd{cos}\hlstd{((Y[}\hlnum{2}\hlstd{]} \hlopt{+} \hlstd{Z[}\hlnum{2}\hlstd{, ])} \hlopt{/} \hlnum{2}\hlstd{)}
    \hlstd{b} \hlkwb{<-} \hlstd{Z[}\hlnum{2}\hlstd{, ]} \hlopt{-} \hlstd{Y[}\hlnum{2}\hlstd{]}

    \hlstd{R} \hlopt{*} \hlkwd{sqrt}\hlstd{(a}\hlopt{^}\hlnum{2} \hlopt{+} \hlstd{b}\hlopt{^}\hlnum{2}\hlstd{)}
\hlstd{\}}
...
\hlkwd{points}\hlstd{(y[}\hlnum{1}\hlstd{], y[}\hlnum{2}\hlstd{],} \hlkwc{col} \hlstd{=} \hlstr{"red"}\hlstd{,} \hlkwc{pch} \hlstd{=} \hlnum{19}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/flat_earth_fn-1} 
