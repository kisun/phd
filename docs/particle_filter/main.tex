...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in lapply(X = X, FUN = FUN, ...): internal error -3 in R\_decompress1}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in lapply(X = X, FUN = FUN, ...): lazy-load database '/home/tom/Documents/uni/phd/docs/particle\_filter/cache/true\_positions\_8e477dc42fd5f5075ae5f49c718f3956.rdb' is corrupt}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): lazy-load database '/home/tom/Documents/uni/phd/docs/particle\_filter/cache/true\_positions\_8e477dc42fd5f5075ae5f49c718f3956.rdb' is corrupt}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in -ncol(z): invalid argument to unary operator}}\begin{alltt}
\hlstd{z} \hlkwb{<-} \hlstd{z[,} \hlopt{-}\hlnum{1}\hlstd{]}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in z[, -1]: incorrect number of dimensions}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `\$<-.data.frame`(`*tmp*`, "{}distance"{}, value = structure(c(0, : replacement has 2 rows, data has 25}}\begin{alltt}
\hlstd{shape1}\hlopt{$}\hlstd{length} \hlkwb{<-} \hlkwd{c}\hlstd{(di,} \hlnum{NA}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `\$<-.data.frame`(`*tmp*`, "{}length"{}, value = structure(c(1653.8564672522, : replacement has 2 rows, data has 25}}\begin{alltt}
...
## 2  174.7973 -36.88168
## 3  174.7972 -36.88074
## 23 174.7818 -36.84780
## 24 174.7742 -36.84535
## 25 174.7678 -36.84393
\end{verbatim}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in if (ncol(a) != ncol(b) \& ncol(a) > 1 \& ncol(b) > 1) stop("{}Incorrent dimensions"{}): argument is of length zero}}\begin{alltt}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in max(shape\$distance): no non-missing arguments to max; returning -Inf}}\begin{alltt}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in max(shape\$distance): no non-missing arguments to max; returning -Inf}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in points(rep(t[i + 1], N), speedhat.hist[, i], pch = 4, col = "{}\#00009930"{}): lazy-load database '/home/tom/Documents/uni/phd/docs/particle\_filter/cache/simulation\_init\_sched\_9b52d8c2c4c2996a54256ac971826a2a.rdb' is corrupt}}\end{kframe}
\end{knitrout}
...
\hlstd{plot.gtfs.block} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{highlight} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{...}\hlstd{)}
    \hlkwd{plot}\hlstd{(x}\hlopt{$}\hlstd{shapes,} \hlkwc{highlight} \hlstd{= highlight, ...)}
...
\hlstd{getBlockAlt} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{ids}\hlstd{,} \hlkwc{date}\hlstd{,} \hlkwc{conn}\hlstd{) \{}
    ...
    \hlstd{fullPath} \hlkwb{<-} \hlkwd{do.call}\hlstd{(rbind, blockShapes[blockInfo}\hlopt{$}\hlstd{shape_id])}
    \hlstd{blockPath} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlkwc{lon} \hlstd{= fullPath}\hlopt{$}\hlstd{lon,} \hlkwc{lat} \hlstd{= fullPath}\hlopt{$}\hlstd{lat,}
                            \hlkwc{trip_id} \hlstd{=} \hlkwd{rep}\hlstd{(blockInfo}\hlopt{$}\hlstd{trip_id,} \hlkwd{sapply}\hlstd{(blockShapes, nrow)))}
    \hlkwd{class}\hlstd{(blockPath)} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"gtfs.shape"}\hlstd{,} \hlkwd{class}\hlstd{(blockPath))}
...
\hlstd{blockPF} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,} \hlkwc{zero.start} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{start.obs} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{...}\hlstd{) \{}
    ...
    \hlstd{pf}
\hlstd{\}}
\hlstd{plot.pf.blockfit} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{obs} \hlstd{=} \hlnum{1}\hlstd{,} \hlkwc{highlight} \hlstd{=} \hlnum{FALSE}\hlstd{) \{}
...
\hlstd{pfit} \hlkwb{<-} \hlkwd{blockPF}\hlstd{(block,} \hlkwc{start.obs} \hlstd{=} \hlnum{5}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{pf2} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,}
    ...
    \hlstd{res}
\hlstd{\}}
\hlstd{fit} \hlkwb{<-} \hlkwd{pf2}\hlstd{(block,} \hlkwc{start.obs} \hlstd{=} \hlnum{5}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{alltt}
\hlkwd{par}\hlstd{(}\hlkwc{mfrow} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{4}\hlstd{,} \hlnum{2}\hlstd{))}
\hlkwa{for} \hlstd{(i} \hlkwa{in} \hlkwd{c}\hlstd{(}\hlnum{2}\hlstd{,} \hlnum{5}\hlstd{,} \hlnum{20}\hlstd{,} \hlnum{30}\hlstd{,} \hlnum{65}\hlstd{,} \hlnum{100}\hlstd{,} \hlnum{190}\hlstd{,} \hlnum{220}\hlstd{))}
    \hlkwd{plot}\hlstd{(fit, i)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=0.8\textwidth]{figure/plot_new_fit-1} 
...
\hlstd{getBlock} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{ids}\hlstd{,} \hlkwc{date}\hlstd{,} \hlkwc{conn}\hlstd{) \{}
    ...
    \hlstd{schedule} \hlkwb{<-} \hlkwd{dbGetQuery}\hlstd{(}
        \hlstd{conn,}
        \hlkwd{sprintf}\hlstd{(}\hlstr{"SELECT st.trip_id, s.stop_id, s.stop_sequence,
