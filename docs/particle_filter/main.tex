...
\hlstd{speedPF} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,}
    ...
    \hlkwa{if} \hlstd{(}\hlkwd{missing}\hlstd{(x0)) \{}
        \hlstd{x0} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{, speed[[schedule}\hlopt{$}\hlstd{trip_id[}\hlnum{1}\hlstd{]]](}\hlnum{0}\hlstd{))}
    \hlstd{\}}
    ...
    \hlstd{B} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwd{rbind}\hlstd{(}\hlkwd{s}\hlstd{(x[}\hlnum{1}\hlstd{]),} \hlnum{0}\hlstd{)}
    \hlstd{G} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{d}\hlstd{)} \hlkwd{rbind}\hlstd{(d}\hlopt{^}\hlnum{2}\hlopt{/}\hlnum{2}\hlstd{, d)}
    \hlstd{fn} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{u}\hlstd{,} \hlkwc{t}\hlstd{,} \hlkwc{w}\hlstd{) \{} \hlcom{## x: [distance, velocity]; u: [delta, time]; w: [error]}
        \hlstd{X} \hlkwb{<-} \hlkwd{structure}\hlstd{(}\hlkwd{A}\hlstd{(u)} \hlopt{%*%} \hlstd{x} \hlopt{+ +} \hlkwd{B}\hlstd{(x)} \hlopt{%*%} \hlstd{u[}\hlnum{1}\hlstd{]} \hlopt{+} \hlkwd{G}\hlstd{(u)} \hlopt{%*%} \hlstd{w,}
                       \hlkwc{.Dimnames} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwd{c}\hlstd{(}\hlstr{"distance"}\hlstd{,} \hlstr{"speed"}\hlstd{),} \hlkwa{NULL}\hlstd{))}
...
\hlstd{fit} \hlkwb{<-} \hlkwd{pf2}\hlstd{(block.speed,} \hlkwc{start.obs} \hlstd{=} \hlnum{1}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{system}\hlstd{(}\hlstr{"convert -delay 2 FIGS/anim*.jpg figure/animated_block_history2.gif"}\hlstd{)}
\hlkwd{unlink}\hlstd{(}\hlstr{"FIGS/*.jpg"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
