...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in points(rep(t[i + 1], N), speedhat.hist[, i], pch = 4, col = "{}\#00009930"{}): internal error -3 in R\_decompress1}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in points(rep(t[i + 1], N), speedhat.hist[, i], pch = 4, col = "{}\#00009930"{}): lazy-load database '/home/tell029/Documents/uni/phd/docs/particle\_filter/cache/simulation\_init\_sched\_9b52d8c2c4c2996a54256ac971826a2a.rdb' is corrupt}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: internal error -3 in R\_decompress1}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): lazy-load database '/home/tell029/Documents/uni/phd/docs/particle\_filter/cache/first\_day\_model\_b02d25c8ba227750ed4f685177503d60.rdb' is corrupt}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in plot(pfit, obs = 30, highlight = 2): internal error -3 in R\_decompress1}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plot(pfit, obs = 30, highlight = 2): lazy-load database '/home/tell029/Documents/uni/phd/docs/particle\_filter/cache/manual\_block\_particleFilter\_3abdea2dc0c5ac666e2ca2c1aa55a78a.rdb' is corrupt}}\end{kframe}
\end{knitrout}
...
\hlstd{getBlock} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{ids}\hlstd{,} \hlkwc{date}\hlstd{,} \hlkwc{conn}\hlstd{) \{}
    ...
    \hlcom{## function from distance to time:}
    \hlstd{f1} \hlkwb{<-} \hlkwd{splinefun}\hlstd{(yy, xx,} \hlkwc{method} \hlstd{=} \hlstr{"hyman"}\hlstd{)}
    \hlcom{## function from time to distance:}
    \hlstd{f2} \hlkwb{<-} \hlkwd{splinefun}\hlstd{(xx, yy,} \hlkwc{method} \hlstd{=} \hlstr{"hyman"}\hlstd{)}
...
\hlstd{accelPF} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,}
    ...
    \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{M) \{}
        ...
        \hlstd{weights} \hlkwb{<-} \hlkwd{apply}\hlstd{(xxhat[,,i}\hlopt{+}\hlnum{1}\hlstd{],} \hlnum{2}\hlstd{,} \hlkwa{function}\hlstd{(}\hlkwc{xi}\hlstd{)} \hlkwd{lhood}\hlstd{(y[,i}\hlopt{+}\hlnum{1}\hlstd{], xi, GPS.error))}
        \hlstd{qi} \hlkwb{<-} \hlstd{weights} \hlopt{/} \hlkwd{sum}\hlstd{(weights)}
        \hlkwa{if} \hlstd{(}\hlkwd{any}\hlstd{(}\hlkwd{is.na}\hlstd{(qi)))}
            \hlstd{qi} \hlkwb{<-} \hlkwa{NULL}
...
\hlstd{\}}
\hlcom{## ------------------------------------------------- The likelihood ##}
\hlstd{lhood} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{y}\hlstd{,} \hlkwc{x}\hlstd{,} \hlkwc{sig}\hlstd{) \{}
                                        \hlcom{# y: GPS observation}
                                        \hlcom{# x: a distance-into-trip}
    \hlkwd{dnorm}\hlstd{(}\hlkwd{distanceFlat}\hlstd{(y,} \hlkwd{h}\hlstd{(x)),} \hlnum{0}\hlstd{,} \hlkwc{sd} \hlstd{= sig)} \hlopt{*}
        \hlkwd{ifelse}\hlstd{(x[}\hlnum{2}\hlstd{]} \hlopt{<=} \hlnum{0}\hlstd{,} \hlnum{0.000001}\hlstd{,} \hlnum{1}\hlstd{)}
...
\hlstd{fn} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{u}\hlstd{,} \hlkwc{w}\hlstd{,} \hlkwc{fun}\hlstd{,} \hlkwc{trips}\hlstd{,} \hlkwc{shape}\hlstd{) \{} \hlcom{## x: [distance, velocity]; u: [delta, time]; w: [error]}
    \hlcom{#s <- function(d, deriv, fn = fun)}
    \hlcom{#    fn$t2d(fn$d2t(d), deriv = deriv + 1)}

    \hlcom{#Xhat <- structure(rbind(x[1, ] + x[2, ] * u[[1]] + 0.5 * s(x[1, ], 1) * u[[1]]^2,}
    \hlcom{#                        x[2, ] + s(x[1, ], 1) * u[[1]],}
    \hlcom{#                        0),}
    \hlcom{#                  .Dimnames = list(c("distance", "speed", "acceleration"), NULL))}
    \hlstd{Xhat} \hlkwb{<-} \hlkwd{structure}\hlstd{(}\hlkwd{rbind}\hlstd{(x[}\hlnum{1}\hlstd{, ]} \hlopt{+} \hlstd{x[}\hlnum{2}\hlstd{, ]} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]]} \hlopt{+} \hlnum{0.5} \hlopt{*} \hlstd{x[}\hlnum{3}\hlstd{, ]} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]]}\hlopt{^}\hlnum{2}\hlstd{,}
                            \hlstd{x[}\hlnum{2}\hlstd{, ]} \hlopt{+} \hlstd{x[}\hlnum{3}\hlstd{, ]} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]],}
                            \hlstd{x[}\hlnum{3}\hlstd{, ]),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fit} \hlkwb{<-} \hlkwd{accelPF}\hlstd{(block.speed,} \hlkwc{N} \hlstd{=} \hlnum{500}\hlstd{,} \hlkwc{sig.e} \hlstd{=} \hlnum{0.5}\hlstd{,} \hlkwc{progress} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlkwc{h} \hlstd{= h,} \hlkwc{lhood} \hlstd{= lhood,} \hlkwc{pw} \hlstd{= pw,} \hlkwc{fn} \hlstd{= fn)}
\end{alltt}
\end{kframe}
\end{knitrout}
