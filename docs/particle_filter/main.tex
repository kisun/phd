...
\hlstd{plot.gtfs.block.shapes} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{highlight}\hlstd{,}
                                   \hlkwc{lty} \hlstd{=} \hlkwd{rep}\hlstd{(}\hlnum{3}\hlstd{,} \hlkwd{length}\hlstd{(x)),}
                                   \hlkwc{...}\hlstd{) \{}
    ...
    \hlkwa{if} \hlstd{(}\hlopt{!}\hlkwd{missing}\hlstd{(highlight))}
        \hlstd{lty[highlight]} \hlkwb{<-} \hlnum{1}

    \hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hlstd{(x))}
        \hlkwd{plot}\hlstd{(x[[i]],} \hlkwc{add} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{lty} \hlstd{= lty[i])}
\hlstd{\}}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{plot}\hlstd{(block.history)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/block_fit_model-1} 

}


\begin{kframe}\begin{alltt}
\hlstd{block.fit} \hlkwb{<-} \hlkwd{particleFilter}\hlstd{(block.history, block}\hlopt{$}\hlstd{path)}
\end{alltt}
\end{kframe}
\end{knitrout}
