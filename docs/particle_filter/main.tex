...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{shapes} \hlkwb{<-} \hlkwd{read.csv}\hlstd{(}\hlstr{"data/at_shapes.txt"}\hlstd{)}
\hlstd{plotShape} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{id}\hlstd{) \{}
    \hlstd{sh} \hlkwb{<-} \hlstd{shapes[shapes}\hlopt{$}\hlstd{shape_id} \hlopt{==} \hlstd{id, ]}
...
\hlkwd{plotShape}\hlstd{(}\hlnum{2}\hlstd{)}
\end{alltt}
\end{kframe}
