...
## 1  174.6470 -36.85062   0.000000  4.196805
## 2  174.6470 -36.85060   4.196805  3.780839
## 3  174.6470 -36.85057   7.977644  0.000000
## 23 174.6479 -36.84944 193.044249 30.725420
## 24 174.6482 -36.84929 223.769669 78.631149
## 25 174.6489 -36.84888 302.400818  9.160502
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\begin{verbatim}
##        lon       lat  distance   length   bearing
## 1 174.6470 -36.85062  0.000000 4.196805  58.00113
## 2 174.6470 -36.85060  4.196805 3.780839 331.92149
## 3 174.6470 -36.85057  7.977644 0.000000   0.00000
## 4 174.6470 -36.85057  7.977644 3.780839 331.92149
## 5 174.6470 -36.85054 11.758484 6.167352 334.35332
## 6 174.6469 -36.85049 17.925836 4.535925 348.68726
\end{verbatim}
\end{kframe}
\end{knitrout}
...
## 1 15:10:00 15:10:00    5960 -36.85062 174.6470       NA
## 2 15:15:24 15:15:24    5105 -36.87119 174.6461       NA
## 3 15:15:53 15:15:53    5003 -36.87472 174.6461       NA
## 4 15:16:36 15:16:36    5005 -36.87663 174.6469       NA
## 5 15:16:53 15:16:53    5007 -36.87880 174.6465       NA
## 6 15:17:04 15:17:04    5107 -36.88055 174.6462       NA
\end{verbatim}
\end{kframe}
\end{knitrout}
...
## 0      0    5960 -36.85062 174.6470
## 300  300    5105 -36.87119 174.6461
## 360  360    5005 -36.87663 174.6469
\end{verbatim}
\begin{alltt}
...
\end{alltt}
\begin{verbatim}
## [1] "20151202" "20151203" "20151207" "20151208" "20151209" "20151210"
## [7] "20151214" "20151215"
\end{verbatim}
\end{kframe}
\end{knitrout}
...
## 1 L459-132 28885378      424  4240026       05:51:00
## 2 L459-132 28886061   441442  4420072       06:27:00
## 3 L459-132 28879378      459  4590011       07:05:00
## 4 L459-132 28886204      459  4590011       07:05:00
## 5 L459-132 28884339      439  4390024       07:10:00
## 6 L459-132 28884452      439  4390026       07:30:00
\end{verbatim}
\begin{alltt}
...
\hlstd{getBlock} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{ids}\hlstd{,} \hlkwc{date}\hlstd{,} \hlkwc{conn}\hlstd{) \{}
    ...
    \hlcom{## schedule$time.seconds <- numeric(nrow(schedule))}
    \hlcom{## speedFns <- tapply(1:nrow(schedule), schedule$trip_id,}
    \hlcom{##                    function(id) \{}
    \hlcom{##                        # Returns the scheduled speed at a given distance-into-trip:}
    \hlcom{##                        d <- schedule$shape_dist_traveled[id]}
    \hlcom{##                        t <- hms(schedule$time[id])}
    \hlcom{##                        tt <- (hour(t) * 60 + minute(t)) * 60}
    \hlcom{##                        t0 <- tt - min(tt)}
    \hlcom{##                        schedule$time.seconds[id] <<- t0}
    \hlcom{##                        xx <- unique(t0)}
    \hlcom{##                        yy <- tapply(d, t0, min)}

    \hlcom{##                        ## function from distance to time:}
    \hlcom{##                        f1 <- splinefun(yy, xx, method = "hyman")}
    \hlcom{##                        ## function from time to distance:}
    \hlcom{##                        f2 <- splinefun(xx, yy, method = "hyman")}

    \hlcom{##                        list(d2t = f1, t2d = f2)}
    \hlcom{##                   \})}
    \hlstd{lens} \hlkwb{<-} \hlkwd{tapply}\hlstd{(schedule}\hlopt{$}\hlstd{shape_dist_traveled, schedule}\hlopt{$}\hlstd{trip_id, max)}
    \hlstd{lens} \hlkwb{<-} \hlstd{lens[}\hlkwd{unique}\hlstd{(schedule}\hlopt{$}\hlstd{trip_id)]}
    \hlstd{lens} \hlkwb{<-} \hlkwd{cumsum}\hlstd{(}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{, lens[}\hlopt{-}\hlkwd{length}\hlstd{(lens)]))}
    \hlkwd{names}\hlstd{(lens)} \hlkwb{<-} \hlkwd{unique}\hlstd{(schedule}\hlopt{$}\hlstd{trip_id)}
    \hlstd{d} \hlkwb{<-} \hlstd{schedule}\hlopt{$}\hlstd{shape_dist_traveled} \hlopt{+} \hlstd{lens[schedule}\hlopt{$}\hlstd{trip_id]}
    \hlstd{t} \hlkwb{<-} \hlkwd{hms}\hlstd{(schedule}\hlopt{$}\hlstd{time)}
    ...
    \hlstd{schedule}\hlopt{$}\hlstd{time.seconds} \hlkwb{<-} \hlstd{t0}
    \hlstd{xx} \hlkwb{<-} \hlkwd{unique}\hlstd{(t0)}
    \hlstd{yy} \hlkwb{<-} \hlkwd{tapply}\hlstd{(d, t0, min)}

    \hlstd{schedule}\hlopt{$}\hlstd{block_dist_traveled} \hlkwb{<-} \hlstd{d}

    \hlcom{## function from distance to time:}
    \hlstd{f1} \hlkwb{<-} \hlkwd{try}\hlstd{(}\hlkwd{splinefun}\hlstd{(yy, xx,} \hlkwc{method} \hlstd{=} \hlstr{"hyman"}\hlstd{),} \hlnum{TRUE}\hlstd{)}
    \hlcom{## function from time to distance:}
    \hlstd{f2} \hlkwb{<-} \hlkwd{try}\hlstd{(}\hlkwd{splinefun}\hlstd{(xx, yy,} \hlkwc{method} \hlstd{=} \hlstr{"hyman"}\hlstd{),} \hlnum{TRUE}\hlstd{)}

    \hlkwa{if} \hlstd{(}\hlkwd{inherits}\hlstd{(f1,} \hlstr{"try-error"}\hlstd{))}
        \hlstd{f1} \hlkwb{<-} \hlkwd{try}\hlstd{(}\hlkwd{splinefun}\hlstd{(yy, xx,} \hlkwc{method} \hlstd{=} \hlstr{"monoH.FC"}\hlstd{),} \hlnum{TRUE}\hlstd{)}

    \hlkwa{if} \hlstd{(}\hlkwd{inherits}\hlstd{(f2,} \hlstr{"try-error"}\hlstd{))}
        \hlstd{f2} \hlkwb{<-} \hlkwd{try}\hlstd{(}\hlkwd{splinefun}\hlstd{(xx, yy,} \hlkwc{method} \hlstd{=} \hlstr{"monoH.FC"}\hlstd{),} \hlnum{TRUE}\hlstd{)}

    \hlkwa{if} \hlstd{(}\hlkwd{inherits}\hlstd{(f1,} \hlstr{"try-error"}\hlstd{)} \hlopt{|} \hlkwd{inherits}\hlstd{(f2,} \hlstr{"try-error"}\hlstd{)) \{}
        \hlstd{speedFns} \hlkwb{<-} \hlkwa{NULL}
        \hlkwd{warning}\hlstd{(}\hlstr{"Cannot fit splines."}\hlstd{)}
    \hlstd{\}} \hlkwa{else} \hlstd{\{}
        \hlstd{speedFns} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlkwc{d2t} \hlstd{= f1,} \hlkwc{t2d} \hlstd{= f2)}
    \hlstd{\}}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fn1} \hlkwb{<-} \hlstd{block.speed}\hlopt{$}\hlstd{speed}
\hlkwd{curve}\hlstd{(fn1}\hlopt{$}\hlkwd{t2d}\hlstd{(x),} \hlnum{0}\hlstd{,}
      \hlkwd{max}\hlstd{(block.speed}\hlopt{$}\hlstd{schedule}\hlopt{$}\hlstd{time.seconds),} \hlkwc{n} \hlstd{=} \hlnum{1001}\hlstd{,}
...
\hlkwd{curve}\hlstd{(fn1}\hlopt{$}\hlkwd{d2t}\hlstd{(x),} \hlnum{0}\hlstd{,}
      \hlstd{mx} \hlkwb{<-} \hlnum{8000}\hlstd{,} \hlkwc{n} \hlstd{=} \hlnum{1001}\hlstd{,} \hlcom{#max(block.speed$schedule$shape_dist_traveled), n = 1001,}
...
\hlkwd{curve}\hlstd{(fn1}\hlopt{$}\hlkwd{t2d}\hlstd{(x,} \hlnum{1}\hlstd{),} \hlnum{0}\hlstd{, tk} \hlopt{*} \hlnum{2}\hlstd{,} \hlkwc{n} \hlstd{=} \hlnum{1001}\hlstd{,}
      \hlkwc{xlab} \hlstd{=} \hlstr{"Time (s)"}\hlstd{,} \hlkwc{ylab} \hlstd{=} \hlstr{"Scheduled Speed (m/s)"}\hlstd{,}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in axis(2, at = c(par()\$yaxp[1:2], sk), labels = c(par()\$yaxp[1:2], : object 'sk' not found}}\begin{alltt}
...
\hlstd{speedPF} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,}
    ...
    \hlkwa{if} \hlstd{(}\hlkwd{missing}\hlstd{(x0)) \{}
        \hlstd{x0} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlkwd{with}\hlstd{(speed,} \hlkwd{t2d}\hlstd{(}\hlkwd{d2t}\hlstd{(}\hlnum{0}\hlstd{),} \hlnum{1}\hlstd{)))}
    \hlstd{\}}
    ...
    \hlstd{B} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{s}\hlstd{) \{}
        \hlstd{sf} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{d}\hlstd{) s}\hlopt{$}\hlkwd{t2d}\hlstd{(s}\hlopt{$}\hlkwd{d2t}\hlstd{(d),} \hlnum{1}\hlstd{)}
        \hlkwd{rbind}\hlstd{(}\hlkwd{sf}\hlstd{(x[}\hlnum{1}\hlstd{, ]),} \hlnum{0}\hlstd{)}
    \hlstd{\}}
    \hlstd{G} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{d}\hlstd{)} \hlkwd{rbind}\hlstd{(d}\hlopt{^}\hlnum{2}\hlopt{/}\hlnum{2}\hlstd{, d)}
    \hlstd{fn} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{u}\hlstd{,} \hlkwc{t}\hlstd{,} \hlkwc{w}\hlstd{) \{} \hlcom{## x: [distance, velocity]; u: [delta, time]; w: [error]}

        \hlstd{X} \hlkwb{<-} \hlkwd{structure}\hlstd{(}\hlkwd{A}\hlstd{(u)} \hlopt{%*%} \hlstd{x} \hlopt{+} \hlkwd{B}\hlstd{(x,} \hlkwc{s} \hlstd{= speed)} \hlopt{*} \hlstd{u[}\hlnum{1}\hlstd{]} \hlopt{+} \hlkwd{G}\hlstd{(u)} \hlopt{%*%} \hlstd{w,}
                       \hlkwc{.Dimnames} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwd{c}\hlstd{(}\hlstr{"distance"}\hlstd{,} \hlstr{"speed"}\hlstd{),} \hlkwa{NULL}\hlstd{))}
        \hlstd{X[}\hlnum{2}\hlstd{, ]} \hlkwb{<-} \hlkwd{ifelse}\hlstd{(X[}\hlnum{2}\hlstd{, ]} \hlopt{<} \hlnum{0}\hlstd{,} \hlnum{0}\hlstd{, X[}\hlnum{2}\hlstd{, ])}
        ...
        \hlstd{stopi} \hlkwb{<-} \hlkwd{which}\hlstd{(shape}\hlopt{$}\hlstd{trip_id} \hlopt{==} \hlstd{tid)}
        \hlstd{Xmax} \hlkwb{<-} \hlkwd{max}\hlstd{(shape[stopi} \hlopt{+} \hlnum{1}\hlstd{,} \hlstr{"distance"}\hlstd{])}
...
\end{verbatim}
\begin{alltt}
\hlkwd{unlink}\hlstd{(}\hlstr{"figure/animated_block_history2.gif"}\hlstd{)}
\hlkwd{system}\hlstd{(}\hlstr{"convert -delay 30 FIGS/anim*.jpg figure/animated_block_history2.gif"}\hlstd{)}
\hlkwd{unlink}\hlstd{(}\hlstr{"FIGS/*.jpg"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\subsection{Use the Integral of Speed}

Instead of using a point speed which from the figures above we can see is absolute rubbish,
we can use the actual scheduled distance covered in time $\Delta_k$, starting at distance $d_{k-1}$.
That is,
\begin{equation}
  \label{eq:integrate_speed}
  \hat d_k - d_{k-1} = 
  \left.\int s\left(t_k\right)\, \mathrm{d}t \right|_{d_{k-1}}^{d_k}
\end{equation}

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{dist2time} \hlkwb{<-} \hlstd{block.speed}\hlopt{$}\hlstd{speed}\hlopt{$}\hlstd{d2t}
\hlstd{time2dist} \hlkwb{<-} \hlstd{block.speed}\hlopt{$}\hlstd{speed}\hlopt{$}\hlstd{t2d}
\hlkwd{curve}\hlstd{(}\hlkwd{time2dist}\hlstd{(x,} \hlnum{1}\hlstd{),} \hlnum{0}\hlstd{,} \hlnum{780}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/integrate_example-1} 

}


\begin{kframe}\begin{alltt}
\hlkwd{integrate}\hlstd{(time2dist,} \hlnum{0}\hlstd{,} \hlnum{780}\hlstd{,} \hlkwc{deriv} \hlstd{=} \hlnum{1}\hlstd{)}
\end{alltt}
\begin{verbatim}
## 4253.554 with absolute error < 0.5
\end{verbatim}
\begin{alltt}
\hlstd{block.speed}\hlopt{$}\hlstd{schedule[}\hlnum{25}\hlstd{,} \hlstr{"shape_dist_traveled"}\hlstd{]}
\end{alltt}
\begin{verbatim}
## [1] 6276.381
\end{verbatim}
...
\hlstd{speedPF} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,}
    ...
    \hlcom{## ---------------------------------------------------------------- ##}
    \hlstd{A} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{d}\hlstd{)} \hlkwd{cbind}\hlstd{(}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{0}\hlstd{),} \hlkwd{c}\hlstd{(d,} \hlnum{0}\hlstd{))}
    \hlstd{B} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{u}\hlstd{,} \hlkwc{s}\hlstd{) \{}
        \hlstd{D} \hlkwb{<-} \hlkwd{cbind}\hlstd{(s}\hlopt{$}\hlkwd{d2t}\hlstd{(x[}\hlnum{1}\hlstd{, ]), s}\hlopt{$}\hlkwd{d2t}\hlstd{(x[}\hlnum{1}\hlstd{, ])} \hlopt{+} \hlstd{u[}\hlnum{1}\hlstd{])}
        \hlstd{d} \hlkwb{<-} \hlkwd{apply}\hlstd{(D,} \hlnum{1}\hlstd{,} \hlkwa{function}\hlstd{(}\hlkwc{t}\hlstd{)} \hlkwd{integrate}\hlstd{(s}\hlopt{$}\hlstd{t2d, t[}\hlnum{1}\hlstd{], t[}\hlnum{2}\hlstd{],} \hlkwc{deriv} \hlstd{=} \hlnum{1}\hlstd{)}\hlopt{$}\hlstd{value)}
        \hlkwd{rbind}\hlstd{(d,} \hlnum{0}\hlstd{)}
    \hlstd{\}}
    \hlstd{G} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{d}\hlstd{)} \hlkwd{rbind}\hlstd{(d}\hlopt{^}\hlnum{2}\hlopt{/}\hlnum{2}\hlstd{, d)}
    \hlstd{fn} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{u}\hlstd{,} \hlkwc{t}\hlstd{,} \hlkwc{w}\hlstd{) \{} \hlcom{## x: [distance, velocity]; u: [delta, time]; w: [error]}

        \hlstd{X} \hlkwb{<-} \hlkwd{structure}\hlstd{(}\hlkwd{A}\hlstd{(u)} \hlopt{%*%} \hlstd{x} \hlopt{+} \hlkwd{B}\hlstd{(x, u,} \hlkwc{s} \hlstd{= speed)} \hlopt{+} \hlkwd{G}\hlstd{(u)} \hlopt{%*%} \hlstd{w,}
                       \hlkwc{.Dimnames} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwd{c}\hlstd{(}\hlstr{"distance"}\hlstd{,} \hlstr{"speed"}\hlstd{),} \hlkwa{NULL}\hlstd{))}
        \hlstd{X[}\hlnum{2}\hlstd{, ]} \hlkwb{<-} \hlkwd{ifelse}\hlstd{(X[}\hlnum{2}\hlstd{, ]} \hlopt{<} \hlnum{0}\hlstd{,} \hlnum{0}\hlstd{, X[}\hlnum{2}\hlstd{, ])}
...
\end{verbatim}
\begin{alltt}
\hlkwd{unlink}\hlstd{(}\hlstr{"figure/animated_block_history3.gif"}\hlstd{)}
\hlkwd{system}\hlstd{(}\hlstr{"convert -delay 30 FIGS/anim*.jpg figure/animated_block_history3.gif"}\hlstd{)}
\hlkwd{unlink}\hlstd{(}\hlstr{"FIGS/*.jpg"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
That sort of worked, but then it didn't.





\section{Schedule-based Acceleration}
...
We also need to ensure that speed is not negative: $v_k > 0\ \forall k$. We'll just do that at the
end of the final function \ldots


\subsection{Defining the Model}

We'll be more efficient now (finally!) and define the likelihood and other funcions outside the main one.

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{accelPF} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{block}\hlstd{,}
                    \hlkwc{N} \hlstd{=} \hlnum{200}\hlstd{,}
                    \hlkwc{zero.start} \hlstd{=} \hlnum{TRUE}\hlstd{,}
                    \hlkwc{start.obs} \hlstd{=} \hlnum{1}\hlstd{,}
                    \hlkwc{x0}\hlstd{,}
                    \hlkwc{GPS.error} \hlstd{=} \hlnum{20}\hlstd{,}
                    \hlkwc{sig.e} \hlstd{=} \hlnum{3}\hlstd{,}
                    \hlkwc{progress} \hlstd{=} \hlnum{FALSE}\hlstd{,}
                    \hlkwc{h}\hlstd{,} \hlkwc{lhood}\hlstd{,} \hlkwc{pw}\hlstd{,} \hlkwc{fn}\hlstd{,}
                    \hlkwc{...}\hlstd{) \{}
    \hlstd{shape} \hlkwb{<-} \hlstd{block}\hlopt{$}\hlstd{path}
    \hlstd{data} \hlkwb{<-} \hlstd{block}\hlopt{$}\hlstd{history[start.obs}\hlopt{:}\hlkwd{nrow}\hlstd{(block}\hlopt{$}\hlstd{history), ]}
    \hlstd{trips} \hlkwb{<-} \hlstd{block}\hlopt{$}\hlstd{trips}
    ...
    \hlkwa{if} \hlstd{(}\hlkwd{missing}\hlstd{(x0)) \{}
        \hlstd{x0} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlkwd{with}\hlstd{(speed,} \hlkwd{t2d}\hlstd{(}\hlkwd{d2t}\hlstd{(}\hlnum{0}\hlstd{),} \hlnum{1}\hlstd{)),} \hlkwd{with}\hlstd{(speed,} \hlkwd{t2d}\hlstd{(}\hlkwd{d2t}\hlstd{(}\hlnum{0}\hlstd{),} \hlnum{2}\hlstd{)))}
    \hlstd{\}}
    ...
    \hlcom{## Matrix prep:}
    \hlstd{xxhat} \hlkwb{<-} \hlstd{xx} \hlkwb{<-} \hlkwd{array}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwd{c}\hlstd{(}\hlnum{3}\hlstd{, N, M} \hlopt{+} \hlnum{1}\hlstd{))}
    \hlstd{hx} \hlkwb{<-} \hlstd{hxhat} \hlkwb{<-} \hlkwd{array}\hlstd{(}\hlnum{NA}\hlstd{,} \hlkwd{c}\hlstd{(}\hlnum{2}\hlstd{, N, M} \hlopt{+} \hlnum{1}\hlstd{))}
    ...
    \hlkwd{invisible}\hlstd{(res)}
\hlstd{\}}

\hlcom{## These stay the same:}
\hlstd{h} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{shape}\hlstd{) \{}
    \hlkwa{if} \hlstd{(}\hlkwd{any}\hlstd{(}\hlkwd{is.na}\hlstd{(x)))} \hlkwd{print} \hlstd{(x)}
...
\hlstd{\}}
\hlcom{## ------------------------------------------------- The likelihood ##}
\hlstd{lhood} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{y}\hlstd{,} \hlkwc{x}\hlstd{,} \hlkwc{sig}\hlstd{,} \hlkwc{shape}\hlstd{) \{}
    \hlcom{## y: GPS observation}
    \hlcom{## x: a distance-into-trip}
    \hlkwd{dnorm}\hlstd{(}\hlkwd{distanceFlat}\hlstd{(y,} \hlkwd{h}\hlstd{(x, shape)),} \hlnum{0}\hlstd{,} \hlkwc{sd} \hlstd{= sig)} \hlopt{*}
        \hlkwd{ifelse}\hlstd{(x[}\hlnum{2}\hlstd{]} \hlopt{<=} \hlnum{0}\hlstd{,} \hlnum{0.000001}\hlstd{,} \hlnum{1}\hlstd{)}
\hlstd{\}}
\end{alltt}
\end{kframe}
...
Now, implementing what we had above:
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## The random component:}
\hlstd{pw} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{n}\hlstd{,} \hlkwc{sig.e}\hlstd{)}
    \hlkwd{rnorm}\hlstd{(n,} \hlnum{0}\hlstd{, sig.e)}

\hlcom{## The formal model ...}
\hlcom{## ---------------------------------------------------------------- ##}
\hlstd{fn} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{u}\hlstd{,} \hlkwc{w}\hlstd{,} \hlkwc{fun}\hlstd{,} \hlkwc{trips}\hlstd{,} \hlkwc{shape}\hlstd{) \{} \hlcom{## x: [distance, velocity]; u: [delta, time]; w: [error]}
    \hlstd{s} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{d}\hlstd{,} \hlkwc{deriv}\hlstd{,} \hlkwc{fn} \hlstd{= fun)}
       \hlstd{fn}\hlopt{$}\hlkwd{t2d}\hlstd{(fn}\hlopt{$}\hlkwd{d2t}\hlstd{(d),} \hlkwc{deriv} \hlstd{= deriv} \hlopt{+} \hlnum{1}\hlstd{)}

    \hlstd{Xhat} \hlkwb{<-} \hlkwd{structure}\hlstd{(}\hlkwd{rbind}\hlstd{(x[}\hlnum{1}\hlstd{, ]} \hlopt{+} \hlstd{x[}\hlnum{2}\hlstd{, ]} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]]} \hlopt{+} \hlnum{0.5} \hlopt{*} \hlkwd{s}\hlstd{(x[}\hlnum{1}\hlstd{, ],} \hlnum{1}\hlstd{)} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]]}\hlopt{^}\hlnum{2}\hlstd{,}
                           \hlstd{x[}\hlnum{2}\hlstd{, ]} \hlopt{+} \hlkwd{s}\hlstd{(x[}\hlnum{1}\hlstd{, ],} \hlnum{1}\hlstd{)} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]],}
                           \hlnum{0}\hlstd{),}
                     \hlkwc{.Dimnames} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwd{c}\hlstd{(}\hlstr{"distance"}\hlstd{,} \hlstr{"speed"}\hlstd{,} \hlstr{"acceleration"}\hlstd{),} \hlkwa{NULL}\hlstd{))}
    \hlcom{## Xhat <- structure(rbind(x[1, ] + x[2, ] * u[[1]] + 0.5 * x[3, ] * u[[1]]^2,}
    \hlcom{##                         x[2, ] + x[3, ] * u[[1]],}
    \hlcom{##                         x[3, ]),}
    \hlcom{##                   .Dimnames = list(c("distance", "speed", "acceleration"), NULL))}
    \hlstd{W} \hlkwb{<-} \hlkwd{rbind}\hlstd{(}\hlnum{0.5} \hlopt{*} \hlstd{w} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]]}\hlopt{^}\hlnum{2}\hlstd{,}
               \hlstd{w} \hlopt{*} \hlstd{u[[}\hlnum{1}\hlstd{]],}
               \hlstd{w)}

    \hlstd{X} \hlkwb{<-} \hlstd{Xhat} \hlopt{+} \hlstd{W}

    \hlstd{X[}\hlnum{1}\hlstd{, ]} \hlkwb{<-} \hlkwd{ifelse}\hlstd{(X[}\hlnum{1}\hlstd{, ]} \hlopt{<} \hlnum{0}\hlstd{,} \hlnum{0}\hlstd{, X[}\hlnum{1}\hlstd{, ])}
    \hlstd{X[}\hlnum{2}\hlstd{, ]} \hlkwb{<-} \hlkwd{ifelse}\hlstd{(X[}\hlnum{2}\hlstd{, ]} \hlopt{<} \hlnum{0}\hlstd{,} \hlnum{0}\hlstd{, X[}\hlnum{2}\hlstd{, ])}

    \hlstd{tid} \hlkwb{<-} \hlstd{trips}\hlopt{$}\hlstd{trip_id[}\hlkwd{which}\hlstd{(trips}\hlopt{$}\hlstd{end_time} \hlopt{>} \hlstd{u[[}\hlnum{2}\hlstd{]])[}\hlnum{1}\hlstd{]]}
    \hlkwa{if} \hlstd{(}\hlkwd{is.na}\hlstd{(tid))}
        \hlkwd{return}\hlstd{(X)}
    ...
    \hlstd{X} \hlkwb{<-} \hlkwd{apply}\hlstd{(X,} \hlnum{2}\hlstd{,} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)} \hlkwa{if} \hlstd{(x[}\hlnum{1}\hlstd{]} \hlopt{>} \hlstd{Xmax)} \hlkwd{c}\hlstd{(Xmax,} \hlnum{0}\hlstd{,} \hlnum{0}\hlstd{)} \hlkwa{else} \hlstd{x)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{past} \hlkwb{<-} \hlstd{hist.blocks[[}\hlnum{3}\hlstd{]][[}\hlnum{12}\hlstd{]]}
\hlstd{block.speed2} \hlkwb{<-} \hlkwd{getBlock}\hlstd{(past}\hlopt{$}\hlstd{trip_id, past}\hlopt{$}\hlstd{trip_start_date[}\hlnum{1}\hlstd{], mbta)}
\hlkwd{plot}\hlstd{(block.speed2)}
\hlstd{block.speed2} \hlkwb{<-} \hlkwd{getBlockData}\hlstd{(block.speed2}\hlopt{$}\hlstd{date,} \hlkwc{block} \hlstd{= block.speed2,} \hlkwc{conn} \hlstd{= mbta)}
\hlkwd{points}\hlstd{(block.speed2)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/acceleration_run-1} 

}


\begin{kframe}\begin{alltt}
\hlstd{fit} \hlkwb{<-} \hlkwd{accelPF}\hlstd{(block.speed2,} \hlkwc{N} \hlstd{=} \hlnum{200}\hlstd{,} \hlkwc{start.obs} \hlstd{=} \hlnum{00}\hlstd{,} \hlkwc{sig.e} \hlstd{=} \hlnum{0.2}\hlstd{,}
               \hlkwc{progress} \hlstd{=} \hlnum{FALSE}\hlstd{,} \hlkwc{h} \hlstd{= h,} \hlkwc{lhood} \hlstd{= lhood,} \hlkwc{pw} \hlstd{= pw,} \hlkwc{fn} \hlstd{= fn)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{with}\hlstd{(block.speed2}\hlopt{$}\hlstd{schedule,} \hlkwd{plot}\hlstd{(time.seconds, block_dist_traveled,} \hlkwc{type} \hlstd{=} \hlstr{"l"}\hlstd{))}
\hlstd{start.time} \hlkwb{<-} \hlkwd{hms}\hlstd{(block.speed2}\hlopt{$}\hlstd{schedule}\hlopt{$}\hlstd{time[}\hlnum{1}\hlstd{])}
\hlstd{start.second} \hlkwb{<-} \hlstd{(}\hlkwd{hour}\hlstd{(start.time)} \hlopt{*} \hlnum{60} \hlopt{+} \hlkwd{minute}\hlstd{(start.time))} \hlopt{*} \hlnum{60} \hlopt{+}
    \hlkwd{as.numeric}\hlstd{(}\hlkwd{as.POSIXct}\hlstd{(block.speed2}\hlopt{$}\hlstd{date,} \hlstr{"EDT5EST"}\hlstd{,} \hlkwc{format} \hlstd{=} \hlstr{"%Y%m%d"}\hlstd{))}
\hlstd{obs.t} \hlkwb{<-} \hlstd{block.speed2}\hlopt{$}\hlstd{history}\hlopt{$}\hlstd{time} \hlopt{-} \hlstd{start.second}
\hlstd{mod.d} \hlkwb{<-} \hlstd{fit}\hlopt{$}\hlstd{Xhat[}\hlnum{1}\hlstd{,,]}
\hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{nrow}\hlstd{(mod.d)) \{}
    \hlkwd{points}\hlstd{(obs.t, fit}\hlopt{$}\hlstd{Xhat[}\hlnum{1}\hlstd{, i, ],} \hlkwc{pch} \hlstd{=} \hlnum{19}\hlstd{,} \hlkwc{cex} \hlstd{=} \hlnum{0.2}\hlstd{,} \hlkwc{col} \hlstd{=} \hlstr{"#00009930"}\hlstd{)}
    \hlkwd{points}\hlstd{(obs.t, fit}\hlopt{$}\hlstd{X[}\hlnum{1}\hlstd{, i, ],} \hlkwc{pch} \hlstd{=} \hlnum{19}\hlstd{,} \hlkwc{cex} \hlstd{=} \hlnum{0.2}\hlstd{,} \hlkwc{col} \hlstd{=} \hlstr{"#00990070"}\hlstd{)}
\hlstd{\}}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/examine-1} 
...
\section{Auckland Transport GTFS Feed}

Finally, as of Wednesday 13 January, 2015, we have data from Auckland Transport!
...
\hlstd{trip090} \hlkwb{<-} \hlstd{athist[athist}\hlopt{$}\hlstd{trip_id} \hlopt{==} \hlstd{athist}\hlopt{$}\hlstd{trip_id[}\hlkwd{grep}\hlstd{(}\hlstr{"^090"}\hlstd{, athist}\hlopt{$}\hlstd{route_id)[}\hlnum{1}\hlstd{]], ]}
\hlstd{obj} \hlkwb{<-} \hlkwd{iNZightMap}\hlstd{(}\hlkwc{lat}\hlstd{=}\hlopt{~}\hlstd{position_latitude,} \hlkwc{lon}\hlstd{=}\hlopt{~}\hlstd{position_longitude,} \hlkwc{data} \hlstd{= trip090,}
                  \hlkwc{name} \hlstd{=} \hlstr{"Route 090 Journey"}\hlstd{)}
\hlkwd{plot}\hlstd{(obj,} \hlkwc{pch} \hlstd{=} \hlnum{4}\hlstd{,} \hlkwc{cex.pt} \hlstd{=} \hlnum{0.5}\hlstd{,} \hlkwc{map.opacity} \hlstd{=} \hlnum{0.4}\hlstd{,} \hlkwc{join} \hlstd{=} \hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{figure/at_history-1} 
...
\hlstd{history} \hlkwb{<-} \hlkwd{dbGetQuery}\hlstd{(}
    ...
    \hlkwd{paste0}\hlstd{(}\hlstr{"SELECT DISTINCT vp.trip_id, vp.vehicle_id, vp.trip_start_date,
            ...
            FROM vehicle_positions AS vp, stop_times AS st, trips AS tr, routes AS rt
            WHERE vp.trip_id=st.trip_id AND st.stop_sequence=1
            AND vp.trip_id=tr.trip_id AND tr.route_id=rt.route_id
            ORDER BY vp.trip_start_date, vp.vehicle_id, trip_start_time"}\hlstd{)}
    \hlstd{)}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in tapply(1:nrow(history), history\$trip\_start\_date, function(i) history[i, : arguments must have same length}}\begin{alltt}
\hlstd{hist.blocks} \hlkwb{<-} \hlkwd{lapply}\hlstd{(hist.dates,} \hlkwa{function}\hlstd{(}\hlkwc{h}\hlstd{) \{}
                          \hlkwd{tapply}\hlstd{(}\hlnum{1}\hlopt{:}\hlkwd{nrow}\hlstd{(h), h}\hlopt{$}\hlstd{vehicle_id,} \hlkwa{function}\hlstd{(}\hlkwc{i}\hlstd{) h[i, ])}
\hlstd{\})}
\end{alltt}
