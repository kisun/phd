...
pfit <- blockPF(block, start.obs = 5)
@ 
...
fit <- pf2(block, start.obs = 5)
@ 

<<plot_new_fit,fig.width=10,fig.height=12,out.width="0.8\\textwidth">>=
#par(mfrow = c(4, 2))
#for (i in c(2, 5, 20, 30, 65, 100, 190, 220))
#for (i in c(2, 3, 4, 10, 30, 40, 50, 60))
...
getShapeDist <- function(sched, sh) {
    ...
    d <- 0
    J <- 1
    for (i in 1:(nrow(sched)-1)) {
        p <- sched[i, c("stop_lon", "stop_lat")]
        for (j in J:nrow(sh)) {
            q1 <- sh[j, c("lon", "lat")]
            q2 <- sh[j+1, c("lon", "lat")]
            v <- q2 - q1
...
getBlock <- function(ids, date, conn) {
    ...
    fullPath <- do.call(rbind, blockShapes[blockInfo$shape_id])
    blockPath <- data.frame(lon = fullPath$lon, lat = fullPath$lat,
                            trip_id = rep(blockInfo$trip_id, sapply(blockShapes, nrow)))
    class(blockPath) <- c("gtfs.shape", class(blockPath))
...
getBlock(blockA$trip_id, blockA$trip_start_date[1], mbta) -> DD
@ 
