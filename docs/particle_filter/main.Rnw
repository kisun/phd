...
getBlock <- function(ids, date, conn) {
    ...
    ## schedule$time.seconds <- numeric(nrow(schedule))
    ## speedFns <- tapply(1:nrow(schedule), schedule$trip_id,
    ##                    function(id) {
    ##                        # Returns the scheduled speed at a given distance-into-trip:
    ##                        d <- schedule$shape_dist_traveled[id]
    ##                        t <- hms(schedule$time[id])
    ##                        tt <- (hour(t) * 60 + minute(t)) * 60
    ##                        t0 <- tt - min(tt)
    ##                        schedule$time.seconds[id] <<- t0
    ##                        xx <- unique(t0)
    ##                        yy <- tapply(d, t0, min)
                           
    ##                        ## function from distance to time:
    ##                        f1 <- splinefun(yy, xx, method = "hyman")
    ##                        ## function from time to distance:
    ##                        f2 <- splinefun(xx, yy, method = "hyman")
                           
    ##                        list(d2t = f1, t2d = f2)
    ##                   })
    lens <- tapply(schedule$shape_dist_traveled, schedule$trip_id, max)
    lens <- lens[unique(schedule$trip_id)]
    lens <- cumsum(c(0, lens[-length(lens)]))
    names(lens) <- unique(schedule$trip_id)
    d <- schedule$shape_dist_traveled + lens[schedule$trip_id]
    t <- hms(schedule$time)
    tt <- (hour(t) * 60 + minute(t)) * 60
    t0 <- tt - min(tt)
    schedule$time.seconds <- t0
    xx <- unique(t0)
    yy <- tapply(d, t0, min)
    
    schedule$block_dist_traveled <- d
    
    ## function from distance to time:
    f1 <- try(splinefun(yy, xx, method = "hyman"), TRUE)
    ## function from time to distance:
    f2 <- try(splinefun(xx, yy, method = "hyman"), TRUE)
    
    if (inherits(f1, "try-error")) 
        f1 <- try(splinefun(yy, xx, method = "monoH.FC"), TRUE)
    
    if (inherits(f2, "try-error"))
        f2 <- try(splinefun(xx, yy, method = "monoH.FC"), TRUE)
        
    if (inherits(f1, "try-error") | inherits(f2, "try-error")) {
        speedFns <- NULL
        warning("Cannot fit splines.")
    } else {
        speedFns <- list(d2t = f1, t2d = f2)
    }
...
<<speed_function_1>>=
fn1 <- block.speed$speed
curve(fn1$t2d(x), 0, 
      max(block.speed$schedule$time.seconds), n = 1001,
      xlab = "Time (s)", ylab = "Distance (m)")
curve(fn1$t2d(x, 1), 0,
      max(block.speed$schedule$time.seconds), n = 1001,
      xlab = "Time (s)", ylab = "Scheduled Speed (m/s)")
@ 
...
<<speed_at_distance,fig.width=8,fig.height=10,out.width="0.6\\textwidth">>=
par(mfrow = c(2, 1))
curve(fn1$d2t(x), 0, 
      mx <- 8000, n = 1001, #max(block.speed$schedule$shape_dist_traveled), n = 1001,
      xlab = "Distance (m)", ylab = "Scheduled Time (s)", 
      yaxs = "i", xaxs = "i", yaxt = "n", xaxt = "n")
dk <- 4000
tk <- fn1$d2t(dk)
axis(1, at = c(par()$xaxp[1:2], dk),
     labels = c(par()$xaxp[1:2], expression(d[k])))
axis(2, at = c(par()$yaxp[1:2], tk),
     labels = c(par()$yaxp[1:2], expression(t[k])), las = 2)
lines(c(rep(dk, 3), 0), c(0, rep(tk, 3)),  lty = 3)

curve(fn1$t2d(x, 1), 0, tk * 2, n = 1001,
      xlab = "Time (s)", ylab = "Scheduled Speed (m/s)", 
      xaxs = "i", xaxt = "n", yaxs = "i", yaxt = "n")
axis(1, at = c(par()$xaxp[1:2], tk),
     labels = c(par()$xaxp[1:2], expression(t[k])))
axis(2, at = c(par()$yaxp[1:2], sk),
     labels = c(par()$yaxp[1:2], expression(s(d[k]))), las = 2)
sk <- fn1$t2d(tk, 1)
lines(c(rep(tk, 3), sk), c(0, rep(sk, 3)), lty = 3)
@ 
...
speedPF <- function(block,
    ...
    B <- function(x, s) {
        sf <- function(d) s$t2d(s$d2t(d), 1)
        rbind(sf(x[1, ]), 0)
    }
    G <- function(d) rbind(d^2/2, d)
    fn <- function(x, u, t, w) { ## x: [distance, velocity]; u: [delta, time]; w: [error]
        
        X <- structure(A(u) %*% x + B(x, s = speed) * u[1] + G(u) %*% w, 
                       .Dimnames = list(c("distance", "speed"), NULL))
        X[2, ] <- ifelse(X[2, ] < 0, 0, X[2, ])
        ...
        stopi <- which(shape$trip_id == tid)
        Xmax <- max(shape[stopi + 1, "distance"])
...
fit <- speedPF(block.speed, start.obs = 1, progress=F)
@ 
...
system("convert -delay 30 FIGS/anim*.jpg figure/animated_block_history2.gif")
unlink("FIGS/*.jpg")
@ 
...
<<integrate_example>>=
dist2time <- block.speed$speed$d2t
time2dist <- block.speed$speed$t2d
curve(time2dist(x, 1), 0, 780)

integrate(time2dist, 0, 780, deriv = 1)
block.speed$schedule[25, "shape_dist_traveled"]
@ 

Close enough for our purposes.

<<speed_function_integral_distance, cache=TRUE>>=
speedPF <- function(block,
                    N = 200,
                    zero.start = TRUE,
                    start.obs = 1,
    ...
    A <- function(d) cbind(c(1, 0), c(d, 0))
    B <- function(x, u, s) {
        D <- cbind(s$d2t(x[1, ]), s$d2t(x[1, ]) + u[1])
        d <- apply(D, 1, function(t) integrate(s$t2d, t[1], t[2], deriv = 1)$value)
        rbind(d, 0)
    }
    G <- function(d) rbind(d^2/2, d)
    fn <- function(x, u, t, w) { ## x: [distance, velocity]; u: [delta, time]; w: [error]
        
        X <- structure(A(u) %*% x + B(x, u, s = speed) + G(u) %*% w, 
                       .Dimnames = list(c("distance", "speed"), NULL))
        X[2, ] <- ifelse(X[2, ] < 0, 0, X[2, ])
...
<<plot_new_fit3,fig.width=10,fig.height=12,out.width="0.8\\textwidth",cache=TRUE>>=
unlink("FIGS/anim*.jpg")
jpeg("FIGS/anim%04d.jpg", width = 400, height = 300)
for(i in 2:length(fit$HX[1,1,]))
    plot(fit, i, main = " ")
...
unlink("figure/animated_block_history3.gif")
system("convert -delay 30 FIGS/anim*.jpg figure/animated_block_history3.gif")
unlink("FIGS/*.jpg")
@ 
...
accelPF <- function(block,
    ...
    if (missing(x0)) {
        x0 <- c(0, with(speed, t2d(d2t(0), 1)), with(speed, t2d(d2t(0), 2)))
    }
    ...
    ## Matrix prep:
    xxhat <- xx <- array(NA, c(3, N, M + 1))
    hx <- hxhat <- array(NA, c(2, N, M + 1))
    ...
    for (i in 1:M) {
        xxhat[,,i+1] <- fn(xx[,,i], list(delta=Delta[i], t=TIME[i+1]), pw(N, sig.e), 
                           fun = speed, trips = trips, shape = shape)
        weights <- try(apply(xxhat[,,i+1], 2, function(xi) lhood(y[,i+1], xi, GPS.error, shape)))
        ...
        hxhat[,,i+1] <- apply(xxhat[,,i+1], 2, h, shape = shape)
        hx[,,i+1] <- apply(xx[,,i+1], 2, h, shape = shape)
        if (progress) setTxtProgressBar(pb, i)
    }
    if (progress) close(pb)
...
## ------------------------------------------------- The likelihood ##
lhood <- function(y, x, sig, shape) {
    ## y: GPS observation
    ## x: a distance-into-trip
    dnorm(distanceFlat(y, h(x, shape)), 0, sd = sig) * 
        ifelse(x[2] <= 0, 0.000001, 1)
}
@ 
...
## The formal model ...
## ---------------------------------------------------------------- ##
fn <- function(x, u, w, fun, trips, shape) { ## x: [distance, velocity]; u: [delta, time]; w: [error]
    s <- function(d, deriv, fn = fun)
       fn$t2d(fn$d2t(d), deriv = deriv + 1)
        
    Xhat <- structure(rbind(x[1, ] + x[2, ] * u[[1]] + 0.5 * s(x[1, ], 1) * u[[1]]^2,
                           x[2, ] + s(x[1, ], 1) * u[[1]],
                           0),
                     .Dimnames = list(c("distance", "speed", "acceleration"), NULL))
    ## Xhat <- structure(rbind(x[1, ] + x[2, ] * u[[1]] + 0.5 * x[3, ] * u[[1]]^2,
    ##                         x[2, ] + x[3, ] * u[[1]],
    ##                         x[3, ]),
    ##                   .Dimnames = list(c("distance", "speed", "acceleration"), NULL))
    W <- rbind(0.5 * w * u[[1]]^2,
    ...
    tid <- trips$trip_id[which(trips$end_time > u[[2]])[1]]
    if (is.na(tid))
        return(X)
    ...
    X <- apply(X, 2, function(x) if (x[1] > Xmax) c(Xmax, 0, 0) else x)
...
<<acceleration_run, cache=TRUE>>=
past <- hist.blocks[[3]][[12]]
block.speed2 <- getBlock(past$trip_id, past$trip_start_date[1], mbta)
plot(block.speed2)
block.speed2 <- getBlockData(block.speed2$date, block = block.speed2, conn = mbta)
points(block.speed2)
fit <- accelPF(block.speed2, N = 200, start.obs = 00, sig.e = 0.2, 
               progress = FALSE, h = h, lhood = lhood, pw = pw, fn = fn)
@ 


<<plot_new_fit4,fig.width=10,fig.height=12,out.width="0.8\\textwidth",cache=TRUE,echo=FALSE,results="hide">>=
unlink("FIGS/anim*.jpg")
jpeg("FIGS/anim%04d.jpg", width = 400, height = 300)
for(i in 2:length(fit$HX[1,1,]))
    plot(fit, i, main = " ")
...
unlink("figure/animated_block_history4.gif")
system("convert -delay 30 FIGS/anim*.jpg figure/animated_block_history4.gif")
unlink("FIGS/*.jpg")
@ 
...
<<examine>>=
with(block.speed2$schedule, plot(time.seconds, block_dist_traveled, type = "l"))
start.time <- hms(block.speed2$schedule$time[1])
start.second <- (hour(start.time) * 60 + minute(start.time)) * 60 + 
    as.numeric(as.POSIXct(block.speed2$date, "EDT5EST", format = "%Y%m%d"))
obs.t <- block.speed2$history$time - start.second
mod.d <- fit$Xhat[1,,]
for (i in 1:nrow(mod.d)) {
    points(obs.t, fit$Xhat[1, i, ], pch = 19, cex = 0.2, col = "#00009930")
    points(obs.t, fit$X[1, i, ], pch = 19, cex = 0.2, col = "#00990070")
}
...
<<at_connection>>=
atcon <- dbConnect(SQLite(), "at_gtfsrt.db")
@ 

<<at_history>>=
library(iNZightMaps)
athist <- dbGetQuery("select distinct * from vehicle_positions", con = atcon)

trip090 <- athist[athist$trip_id == athist$trip_id[grep("^090", athist$route_id)[1]], ]
obj <- iNZightMap(lat=~position_latitude, lon=~position_longitude, data = trip090,
                  name = "Route 090 Journey")
plot(obj, pch = 4, cex.pt = 0.5, map.opacity = 0.4, join = TRUE)
@ 


<<at_firstattempt>>=
## Get a trip set for a vehicle:
