...
curve(fn1$t2d(x), 0, 
      max(block.speed$schedule[block.speed$schedule$trip_id == 
                                   block.speed$trips$trip_id[1], ]$time.seconds),
...
par(mfrow = c(2, 1))
curve(fn1$d2t(x), 0, 
      mx <- max(block.speed$schedule[block.speed$schedule$trip_id == 
...
speedPF <- function(block,
    ...
    if (missing(x0)) {
        x0 <- c(0, with(speed[[schedule$trip_id[1]]], t2d(d2t(0), 1)))
    }
    ...
    B <- function(x, sfns) {
        s <- function(d) sfns$t2d(sfns$d2t(d), 1)
        rbind(s(x[1]), 0)
    }
    G <- function(d) rbind(d^2/2, d)
    fn <- function(x, u, t, w) { ## x: [distance, velocity]; u: [delta, time]; w: [error]
        tid <- trips$trip_id[which(trips$end_time > t)[1]]
        print(B(x, sfns = speed[[tid]]))
        X <- structure(A(u) %*% x + B(x, sfns = speed[[tid]]) %*% u[1] + G(u) %*% w, 
                       .Dimnames = list(c("distance", "speed"), NULL))
        if (is.na(tid))
            return(X)
        Xmax <- max(shape[shape$trip_id == tid, "distance"])
...
fit <- speedPF(block.speed, start.obs = 1)
@ 
