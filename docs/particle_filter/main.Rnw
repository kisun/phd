...
thePlot3 <- function(x1, x2) {
    plotShape(shapeIDs[1, 1], con)
    if (!missing(x1)) points(x1[1, ], x1[2, ], pch = 3, col = "#00009970")
    if (!missing(x2)) points(x2[1, ], x2[2, ], pch = 3, cex = 0.5, col = "#00990040")
...
N <- 300 ## particles
x0 <- t(rmvnorm(N, c(0, 5), diag(c(0, 5)))) # should really just use rbind(0, rnorm()) ...
