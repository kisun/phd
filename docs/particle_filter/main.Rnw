...
<<mbta_getdata,cache=TRUE>>=
con2 <- function() dbConnect(SQLite(), "mbta_gtfs_historical.db")
## we know route_type 3 = bus
all.routes <- dbGetQuery(con2(), 
                         "SELECT route_id, route_long_name 
                          FROM routes WHERE route_type=3")
head(all.routes)
@ 
Let's checkout Route 746 - Silver Line Waterfront
<<checkout_routes,cache=TRUE>>=
route.shapes <- dbGetQuery(con2(), 
                           "SELECT * FROM shapes 
                            WHERE shape_id IN (SELECT DISTINCT shape_id 
                                               FROM trips 
                                               WHERE route_id='746' LIMIT 1)") 
head(route.shapes)
shapeID <- unique(route.shapes$shape_id)
plotShape(shapeID, con2())
@
Checkout some data on it!
<<get_trip_history,cache=TRUE>>=
shape.history <- dbGetQuery(
    con2(),
    sprintf("SELECT trip_id, COUNT(*) AS n FROM vehicle_positions 
             WHERE trip_id IN (SELECT trip_id FROM trips 
             WHERE shape_id='%s') 
             GROUP BY trip_id ORDER BY n DESC",
            shapeID))
head(shape.history)

trip.history <- dbGetQuery(
    con2(),
    sprintf("SELECT trip_id, trip_start_date, 
             position_latitude AS lat, position_longitude AS lon, timestamp 
             FROM vehicle_positions 
             WHERE trip_id IN ('%s')
             ORDER BY trip_id, timestamp",
            paste0(shape.history$trip_id, collapse="','")))
head(trip.history)

@

Alright, how about just one day, one trip:
<<one_dat>>=
tripID <- "27638495"
date <- "20150827"
data <- trip.history[trip.history$trip_id == tripID & 
                     trip.history$trip_start_date == date, ]
head(data)
shape2 <- plotShape(shapeID, con2())
points(shape2$lon[1], shape2$lat[1], pch = 19, cex=0.5)
points(data$lon, data$lat, pch = 4, col = "red")

data$time <- data$timestamp - min(data$timestamp)
@ 
\end{document}
