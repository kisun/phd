...
accelPF <- function(block,
    ...
    for (i in 1:M) {
        ...
        weights <- apply(xxhat[,,i+1], 2, function(xi) lhood(y[,i+1], xi, GPS.error))
        qi <- weights / sum(weights)
        if (any(is.na(qi)))
            qi <- NULL
...
## ------------------------------------------------- The likelihood ##
lhood <- function(y, x, sig) {
                                        # y: GPS observation
                                        # x: a distance-into-trip
    dnorm(distanceFlat(y, h(x)), 0, sd = sig) * 
        ifelse(x[2] <= 0, 0.000001, 1)
...
<<acceleration_run>>=
fit <- accelPF(block.speed, N = 4, sig.e = 0.5, progress = TRUE, h = h, lhood = lhood, pw = pw, fn = fn)
@ 
