...
plotShape <- function(id) {
    sh <- dbGetQuery(con, sprintf("SELECT shape_pt_lon, shape_pt_lat FROM shapes WHERE shape_id='%s'", id))
...
distanceFlat <- function(y, z, R = 6371000) {
    ...
    if (length(dim(z)) < 2)
        z <- cbind(z)
    #R * sqrt(((y[1] - z[1, ]) * cos((y[2] + z[2, ]) / 2))^2 + (y[2] - z[2, ])^2)
    
    Y <- y * pi / 180
    Z <- z * pi / 180
    a <- (Z[1,] - Y[1]) * cos((Y[2] + Z[2, ]) / 2)
    b <- Z[2, ] - Y[2]
