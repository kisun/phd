...
print.triphistory <- function(x, ...) {
    print.data.frame(x)
    cat("\n")
    print(structure(c(attr(x, "date"), attr(x, "trip_id")), .Names = c("date", "trip_id")))
}
...
getBlock <- function(id, conn) {
    blockInfo <- dbGetQuery(
        conn,
        sprintf("SELECT block_id, trip_id, route_id, shape_id
                 FROM trips WHERE block_id='%s'", bID))
...
head.gtfs.block <- function(x, n = 6, ...)
    head(x$trips, n, ...)
plot.gtfs.block <- function(x, ...)
    plot(x$shapes)
plot.gtfs.block.shapes <- function(x, highlight, 
                                   lty = rep(3, length(x)), 
                                   ...) {
...
getBlockData <- function(date, id, conn) {
    b <- getBlock(id, conn)
    #d <- lapply(b$trips$trip_id, function(ID) getTripData(date, ID, conn))
    d <- dbGetQuery(
        conn,
        sprintf("SELECT position_latitude AS lat, position_longitude AS lon,
                 ...
                 ORDER BY timestamp",
                paste0(b$trips$trip_id, collapse = ","), date))
    
    class(d) <- c("blockhistory", "triphistory", class(d))
    d
}
block.history <- getBlockData("20150820", bID, mbta)
@ 

<<block_fit_model,cache=TRUE>>=
plot.pf.blockfit <- function(x, obs = 1, conn = con) {
    plot(block)
    points(x$HXhat[1,,obs], x$HXhat[2,,obs], pch = 4, col = "#00009930")
    points(x$HX[1,,obs], x$HX[2,,obs], pch = 4, cex = 0.5, col = "#00990070")
    points(x$y[1,obs], x$y[2,obs], col = "red", pch = 19, cex = 0.4)
...
block.fit <- particleFilter(block.history, block$path)
class(block.fit) <- c("pf.blockfit", class(block.fit))
