...
\begin{frame}{Motivation}
  ...
  \item Prediction models not that good (at least in Auckland)
    \begin{itemize}
    \item Input: AVL data, scheduling information? historical? (black box)
    \item Doesn't cope with many situations: major delays, no bus
    \end{itemize}

  \item \emph{Improved} bus prediction
    \begin{itemize}[<1->]
    \item more accurate predictions: model, ``real-time'' data
    \item prediction intervals
    \item accessibility: UI, journey planning
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Recursive Bayesian Filtering: Particle Filter II}
  Transform particle's distance into a lat/lon coordinate using shape information \ldots
  
  \begin{tikzpicture}[remember picture]
    \node[at=(current page.center)] {
      \includegraphics[width=0.5\textwidth]{gps-dist2.jpg}
      \includegraphics[width=0.5\textwidth]{gps-dist1.jpg}
    };
  \end{tikzpicture}        
  
  Likelihood now the distance between two points.
\end{frame}
...
\begin{frame}{Predicting Arrival Times: Historical Data}
  ...
  \item can build up models: day of the week, time of day, etc
...
\begin{frame}{Predicting Arrival Times: Real-time Data}  
  \begin{itemize}
  \item Real-time data for up to 1000+ Auckland buses
  \item Particle Filter provides distribution of speed at coordinates, $v_k$
  \item \cite{yu-etal:2011}: bus stops with multiple routes\\
    e.g., along Symonds St
  
    \vspace{1em}
  \item<2-> \textbf{Proposal}: combine travel times and speed estimates of \emph{all} buses
    \begin{itemize}
...
\begin{frame}{Communicating Predictions}
  ...
  \item how to convey \emph{distribution of particles}?
