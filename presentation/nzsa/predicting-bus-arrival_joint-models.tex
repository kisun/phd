...
%\subtitle{}
\author{Tom Elliott\\\scriptsize{\url{tomelliott.co.nz}}}
\date{}
...
\begin{frame}
  ...
  \only<2>{
    $\dagger$ Dealing specifically with Auckland Transport
  ...
  \item<8-> Recent modelling frameworks \emph{don't} make use of 
...
\begin{frame}
  ...
  \item<+-> \textbf{Transition} each particle independently
    ...
    \item<5-> What about intermediate stop(s)?
      ...
      \item<6-> \textbf{Dwell time} $ = p_{s_k}^{(i)} \left(\gamma + \bar t_{s_k}\right)$
...
\begin{frame}
  ...
  \item<4-> intuitive\footnote{arguably}: ETA 6~min (mean) versus ETA 3--8~min
