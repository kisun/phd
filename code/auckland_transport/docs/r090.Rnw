...
draw <- function(s = 0, a, labs = TRUE, xlim = NULL, ylim = NULL, stop.pch = 19) {
    with(hist, plot(time_into_trip, distance, type = "n",
                    main = ifelse(labs, "History of Route 090, 12:15 trip and previous 11:15 trip", ""), 
                    xlab = ifelse(labs, "Time into Trip (s)", ""),
                    ylab = ifelse(labs, "Distance into Trip (m)", ""),
                    xlim = xlim, ylim = ylim,
                    axes = labs))
