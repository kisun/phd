...
for (i in 1:length(days)) {
    ...
    plot(NA, xlim = range(t), ylim = c(0, Ymax), 
         xlab = "Time into Trip (s)",
         ylab = "Distance into Trip (m)")
    lines(t, d1 <- DIST[[i]][1, ], col = "#990000", lwd = 2)
    lines(t, d2 <- DIST[[i]][2, ], col = "#000099", lwd = 2)
    
    par(mar = c(5.1, 0, 4.1, 4.1))
    plot(NA, ylim = c(0, Ymax), xlim = c(0, Vmax),
         xlab = "Speed (m/s)", yaxt = "n")
    lines(s1 <- SPEED[[i]][1, ], d1, col = "#990000", lwd = 1, lty = 3)
    lines(s2 <- SPEED[[i]][2, ], d2, col = "#000099", lwd = 1, lty = 3)
    dn1 <- lowess(s1, f = 1/5)
    dn2 <- lowess(s2, f = 1/5)
