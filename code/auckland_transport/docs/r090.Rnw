...
<<extract_historical_data,fig.height=5,fig.width=9,cache=TRUE>>=
routeid <- "09001%"
...
<<example_trip,fig.height=5,fig.width=9,cache=TRUE>>=
tripid <- "^3090020609"
...
<<spline_curves,fig.height=5,fig.width=9,cache=TRUE>>=
history.fn <- try({
...
<<stop_one_arrival,fig.width=9,fig.height=5,cache=TRUE>>=
...
<<stops_remaining_arrival,fig.width=9,fig.height=5,cache=TRUE>>=
...
<<first_observation,fig.width=9,fig.height=5,cache=TRUE>>=
...
<<multiple_route_history,fig.width=9,fig.height=5,cache=TRUE>>=
with(hist090, plot(time.hour, distance, type = "n",
                   main = "History of Route 090", xlab = "Time (h)",
                   ylab = "Distance into Trip (m)"))
invisible(tapply(1:nrow(hist090), hist090$dvt,
...
<<historical_sameday,fig.width=9,fig.height=5,cache=TRUE>>=
## the trips
...
<<supporting_functions,fig.width=9,fig.height=5,cache=TRUE>>=
rpred.fn <- try({
...
<<prediction_function,fig.height=5,fig.width=9,cache=TRUE>>=
predict <- function(i, draw = TRUE, ...) {
    ...
    if (sum(Pi) == 0) {
        w0[ , , i] <- runif(M, 0, max = max(w0[, , i]))
        Pi <- dnorm(w0[1,,i], test.trip$distance[i], 20)
        if (sum(Pi) == 0) {
            w0[ , , i] <- runif(M, 0, max = 2 * max(w0[, , i]))
            Pi <- dnorm(w0[1,,i], test.trip$distance[i], 20)
        }
    }
    wi <- Pi / sum(Pi)
...
<<examine_relation,cache=TRUE>>=
## Obtain the data for each trip
r1 <- hist090[hist090$trip_start_time == "11:15:00", ]
r2 <- hist090[hist090$trip_start_time == "12:15:00", ]

## Distribute into days:
days <- unique(r1$trip_start_date)[unique(r1$trip_start_date) %in% 
                                   unique(r2$trip_start_date)]
hist.paired <- lapply(days, function(d) {
    t1 <- r1[r1$trip_start_date == d, c("time.zero", "distance")]
    t2 <- r2[r2$trip_start_date == d, c("time.zero", "distance")]
    d1 <- splinefun(t1$time.zero, t1$distance, method = "hyman", ties = "min")
    d2 <- splinefun(t2$time.zero, t2$distance, method = "hyman", ties = "min")
    list(r1 = d1, r2 = d2)
})
names(hist.paired) <- days

t <- seq(0, max(r1$time.zero, r2$time.zero), length = 1001)
DIST <- lapply(hist.paired, function(d) {
    rbind(d$r1(t), d$r2(t))
})
SPEED <- lapply(hist.paired, function(d) {
    rbind(d$r1(t, deriv = 1), d$r2(t, deriv = 1))
})
Ymax <- max(hist090$distance)
Vmax <- 50
@ 

<<examine_relation_GIF,cache=TRUE>>= 
plotfn <- function(i) {
    ti1 <- range(r1[r1$trip_start_date == days[i], "time.zero"])
    ti2 <- range(r2[r2$trip_start_date == days[i], "time.zero"])
    tw1 <- which(t >= ti1[1] & t <= ti1[2])
    tw2 <- which(t >= ti2[1] & t <= ti2[2])
    
    layout(rbind(c(1,2, 3)), width = c(10, 2, 2))
    par(mar = c(5.1, 4.1, 4.1, 0))    
    plot(NA, xlim = range(t), ylim = c(0, Ymax), 
         xlab = "Time into Trip (s)",
         ylab = "Distance into Trip (m)")
    lines(t[tw1], d1 <- DIST[[i]][1, tw1], col = "#990000", lwd = 2)
    lines(t[tw2], d2 <- DIST[[i]][2, tw2], col = "#000099", lwd = 2)
    
    par(mar = c(5.1, 0, 4.1, 0))
    plot(NA, ylim = c(0, Ymax), xlim = c(0, Vmax),
         xlab = "Speed (m/s)", yaxt = "n")
    lines(s1 <- SPEED[[i]][1, tw1], d1, col = "#990000", lwd = 1, lty = 3)
    lines(s2 <- SPEED[[i]][2, tw2], d2, col = "#000099", lwd = 1, lty = 3)
    dn1 <- lowess(s1, f = 1/5)
    dn2 <- lowess(s2, f = 1/5)
    lines(dn1$y, d1, col = "#990000", lwd = 2)
    lines(dn2$y, d2, col = "#000099", lwd = 2)
    axis(1)
    
    tw <- tw1[which(tw1 %in% tw2)]
    speed.diff <- apply(SPEED[[i]][, tw], 2, diff)
    dist.both <- DIST[[i]][1, tw]
    speed.smooth <- lowess(speed.diff, f = 1/10)
    
    par(mar = c(5.1, 0, 4.1, 4.1))
    plot(NA, ylim = c(0, Ymax), xlim = range(speed.diff),
         xlab = "Speed differential", yaxt = "n")
    abline(v = 0, lty = 3)
    lines(speed.diff, dist.both, col = "#009900", lwd = 1, lty = 3)
    lines(speed.smooth$y, dist.both, col = "#009900", lwd = 2)
    axis(1)
    axis(4) 
}
jpeg("hist/route090_1115_1215_%02d.jpg", width = 120*9, height = 120*5)
...
<<collection_corr_trips,fig.width=11,fig.height=5,out.width="0.8\\textwidth",fig.cap='A selection of historical trips, comparing 11:15am and 12:15pm trips.',fig.env='figure',fig.subcap=days[c(3, 8, 11)],results='hide',fig.align="center">>=
for( i in c(3, 8, 11) ) plotfn(i)
@ 


%Actual correlation?
<<eval=FALSE,echo=FALSE,speed_correlation,fig.width=5,fig.height=5,out.width="0.3\\textwidth",fig.cap='Speed at equal distance into trip',fig.env='figure',fig.subcap=days>>=
for (i in 1:length(days)) {
    ti1 <- range(r1[r1$trip_start_date == days[i], "time.zero"])
    ti2 <- range(r2[r2$trip_start_date == days[i], "time.zero"])
    tw1 <- which(t >= ti1[1] & t <= ti1[2])
    ...
    tw <- tw1[which(tw1 %in% tw2)]
    sp <- SPEED[[i]][, tw]
    plot(sp[1, ], sp[2, ], xlab = "Speed 11.15", ylab = "Speed 12.15",
         pch = 19, col = "#00000050", main = round(cor(t(sp))[1, 2], 2))
}
@ 
...
<<compare_historical_speeds,fig.width=9,fig.height=5,cache=TRUE>>=
tripHistPlot <- function(col = rgb(0,0,0,1), h, v, hist.cols = rep(col, nrow(HIST))) {
    ti <- do.call(rbind, lapply(days, function(d) range(r1[r1$trip_start_date == d, "time.zero"])))
    ti <- c(max(ti[, 1]), min(ti[, 2]))
    tw <- which(t >= ti[1] & t <= ti[2])
    hist.dists <- do.call(rbind, lapply(DIST, function(d) d[1, tw]))
    HIST <- hist.dists[-nrow(hist.dists), ]
    PRED <- hist.dists[nrow(hist.dists), ]
    hist.speeds <- do.call(rbind, lapply(SPEED, function(s) s[1, tw]))
    HIST1 <- hist.speeds[-nrow(hist.speeds), ]
    PRED1 <- hist.speeds[nrow(hist.speeds), ]
    
    layout(rbind(c(1, 2)), widths = c(4, 1))
    par(mar = c(5.1, 4.1, 4.1, 0))
    plot(NA, xlim = range(t[tw]), ylim = range(hist.dists),
         xlab = "Time into Trip (s)", ylab = "Distance into Trip (m)")
    sapply(1:nrow(HIST), 
           function(i) lines(t[tw], HIST[i, ], lty = 3, col = hist.cols[i]))
    lines(t[tw], PRED, col = "#990000", lwd = 2)
    
    if (!missing(h)) abline(h = h, col = "#009900", lty = 3)
    if (!missing(v)) abline(h = h, col = "#009900", lty = 3)
    
    par(mar = c(5.1, 0, 4.1, 4.1))
    plot(NA, xlim = range(hist.speeds), ylim = range(hist.dists),
         xlab = "Speed (m/s)", yaxt = "n")
    axis(4)
    sapply(1:nrow(HIST), function(i) lines(HIST1[i, ], HIST[i, ], lty = 3))
    lines(PRED1, PRED, col = "#990000", lwd = 2)
    if (!missing(h)) abline(h = h, col = "#009900", lty = 3)
}
tripHistPlot()
@ 
...
<<historical_speeds_5000m,cache=TRUE>>=
D5000 <- sapply(hist.paired, function(h) {
    ## convert distance to time, time to ["speed", "acceleration"]
    t <- optimize(
        function(x) (h$r1(x) - 5000)^2,
        interval = c(0, 1500)
    )$minimum
    h$r1(t, deriv = 1)
})
vhist <- D5000[-length(D5000)]
vpred <- D5000[length(D5000)]
...
<<speed_diff_5000m,fig.width=9,fig.height=5,cache=TRUE>>=
pr <- dnorm(vpred - vhist, sd = sd(vhist))
wt <- pr / sum(pr)
tripHistPlot(col = rgb(0,0,0, wt / max(wt)), h = 5000)
@ 



<<weighted-predictions,fig.width=9,fig.height=5,cache=TRUE>>=
N <- nrow(test.trip)
...
colWtMeans <- function(x, wt, ...)
    apply(x, 2, function(x) sum(x * wt, ...) / sum(wt, ...))

pred <- history.fnD(w[1, ], delta.t[2])
pred.v <- history.fnD(w[1, ], 0, 1)
wt <- rep(1, nrow(pred.v))

w0[2, ] <- rtnorm(ncol(w), colWtMeans(pred, wt), apply(pred, 2, sd), lower = w[1, ])
Pi <- dnorm(w0[2, ], test.trip$distance[2], 20)
wi <- Pi / sum(Pi)
w[2, ] <- sample(w0[2, ], M, TRUE, wi)
draw(1, a = a[2, ], xlim = c(0, 1000), ylim = c(0, 5200))
...
adj <- - sweep(arrivalTime(w[2, ]), 1, a1j)
adj[adj < 0] <- 0
a1 <- ti + rtnorm(M, colWtMeans(adj, wt, na.rm = TRUE),
                  colSds(adj, na.rm = TRUE), lower = 0)
draw(1, a = a1)
...
for (i in 2:15) {
    ...
    adj <- - sweep(arrivalTime(w[2, ]), 1, aij)
    adj[adj < 0] <- 0
    a[i, ] <- ti + rtnorm(M, colWtMeans(adj, wt), 
                          colSds(adj), lower = 0)
}

draw <- function(s = 0, a, labs = TRUE, xlim = NULL, ylim = NULL, 
                 stop.pch = 19, stop.col = "red", hist.weights) {
    with(hist, plot(time_into_trip, distance, type = "n",
                    main = ifelse(
                        labs, 
                        "History of Route 090 12:15pm trip", 
                        ""), 
                    xlab = ifelse(labs, "Time into Trip (s)", ""),
                    ylab = ifelse(labs, "Distance into Trip (m)", ""),
                    xlim = xlim, ylim = ylim,
                    axes = labs))
    box()
    if (missing(hist.weights)) hist.weights <- rep(1, length(history.fn))
    else hist.weights <- as.numeric(hist.weights) / max(hist.weights)

    for (i in 1:length(history.fn)) {
        f <- history.fn[[i]]
        curve(f(x), 0, attr(f, "max"), n = 101, add = TRUE, lty = 2, 
              col = rgb(0, 0, 0, hist.weights[i]))
    }
    if (any(s == 0))
        abline(h = c(0, stops), col = "#0000ff", lty = 3)
    else if (all(s > 0))
...
predict <- function(i, draw = TRUE, ...) {
    pred <- history.fnD(w[i - 1, ], delta.t[i])
    pred.v <- history.fnD(w[i - 1, ], 0, 1)
    cur.v <- mean((w[i - 1, ] - w[i - 2, ]) / delta.t[i - 1])
    pr <- dnorm(pred.v - cur.v, sd = sd(pred.v))
    wt <- sweep(pr, 2, colSums(pr), "/")
    wtMeans <- colSums(pred * wt) / colSums(wt)
        
    w0[i, ] <<- rtnorm(ncol(w), wtMeans, apply(pred, 2, sd), lower = w[i - 1, ])
    Pi <- dnorm(w0[i, ], test.trip$distance[i], 20)
    if (all(Pi == 0)) {
        w0[i, ] <<- runif(ncol(w), 0, max(w0[i, ]))
        Pi <- dnorm(w0[i, ], test.trip$distance[i], 20)
    }
    wi <- Pi / sum(Pi)
    w[i, ] <<- sample(w0[i, ], M, TRUE, wi)
    ...
    adj <- - sweep(arrivalTime(w[i, ]), 1, a1j)
    adj[adj < 0] <- 0
    wtMeans <- colSums(adj * wt) / colSums(wt)
    a[1, ] <- ti + rtnorm(M, ifelse(wtMeans == 0, 10, wtMeans), 
                          colSds(adj), lower = 0)
    a[1, wtMeans == 0] <- pmax(0, a[1, wtMeans == 0] - 10)
    ...
    for (j in 2:length(stops)) {
        ...
        adj[adj < 0] <- 0
        wtMeans <- colSums(adj * wt) / colSums(wt)
        a[j, ] <- ti + rtnorm(M, ifelse(wtMeans == 0, 10, wtMeans), 
                              colSds(adj), lower = 0)
        a[j, wtMeans == 0] <- pmax(0, a[j, wtMeans == 0] - 10)
    }
    if (draw) {
        Wt <- rowSums(wt) / sum(wt)
        
        draw(wstops, a, hist.weights = Wt, xlim = c(0, 6000), ...)
...
<<eval=FALSE>>=
pb <- txtProgressBar(3, nrow(test.trip), initial = 3, style = 3)
jpeg("hist/route090_1215-weighted_%03d.jpg", width = 900, height = 500)
for (i in 3:nrow(test.trip)) {
   predict(i)
   setTxtProgressBar(pb, i)
   Sys.sleep(1)
};dev.off();close(pb)
unlink("hist/route090_1215-weighted.gif")
system("convert -delay 30 hist/route090_1215-weighted_*.jpg hist/route090_1215-weighted.gif")
@ 

It's time we attempt to obtain real bus stops.
<<real_stops,fig.width=9,fig.height=5,cache=TRUE>>=
Shape <- getPattern(id = test.trip$trip_id[1], con = "gtfs-static.db", 
                    verbose = FALSE)
Sched <- getSchedule(id = test.trip$trip_id[1], con = "gtfs-static.db",
                     verbose = FALSE)
Sched$distance_into_trip <- getShapeDist(Sched, Shape)

stops <- Sched$distance_into_trip
draw(1:length(stops))
@ 

<<analyse_real_stops,fig.width=9,fig.height=5,cache=TRUE>>=
N <- nrow(test.trip)
w <- matrix(nrow = N, ncol = M)
w[1, ] <- 0
w0 <- w  ## matrix for the original estimates
...
w0[2, ] <- rtnorm(ncol(w), colWtMeans(pred, wt), 
                  apply(pred, 2, function(x) sd(x, na.rm = TRUE) / sqrt(sum(!is.na(x)))), 
                        lower = w[1, ])
Pi <- dnorm(w0[2, ], test.trip$distance[2], 20)
wi <- Pi / sum(Pi)
w[2, ] <- sample(w0[2, ], M, TRUE, wi)
...
<<eval=FALSE>>=
pb <- txtProgressBar(3, nrow(test.trip), initial = 3, style = 3)
jpeg("hist/route090_1215-weighted-realstops_%03d.jpg", width = 900, height = 500)
for (i in 3:nrow(test.trip)) {
   predict(i)
   setTxtProgressBar(pb, i)
   Sys.sleep(1)
};dev.off();close(pb)
unlink("hist/route090_1215-weighted-realstops.gif")
system(paste0("convert -delay 30 hist/route090_1215-weighted-realstops_*.jpg ",
              "hist/route090_1215-weighted-realstops.gif"))
@ 
...
<<raw_route_090_history,fig.width=9,fig.height=5,cache=TRUE>>=
CURRENT.ID <- test.trip$trip_id[1]
CURRENT.start <- min(hist090$timestamp[hist090$trip_id == CURRENT.ID])
HIST <- hist090[hist090$timestamp < CURRENT.start, ]
HIST$dvt <- factor(HIST$dvt)
with(HIST, plot(time.hour, distance, type = "n",
                main = "History of Route 090", xlab = "Time (h)", 
                ylab = "Distance into Trip (m)"))
invisible(tapply(1:nrow(HIST), HIST$dvt, 
                 function(i) lines(HIST$time.hour[i], HIST$distance[i], 
                                   col = rgb(0,0,0,0.4), lty = 3)))
with(test.trip, lines(time.hour, distance, col = "#990000", lwd = 2, lty = 2))
@ 
...
<<compute_arrival_times_history,cache=TRUE,fig.width=9,fig.height=5>>=
histArrivalTime <- function(distance, history) {
    fns <- try({
        tapply(1:nrow(history), history$dvt, function(i) {
                   f <- splinefun(history$time.zero[i],
                                  history$distance[i],
                                  method = "hyman", ties = min)
                   attr(f, "max") <- max(history$time.zero[i])
                   attr(f, "start.time") <- history$trip_start_time[i[1]]
                   attr(f, "start.hour") <- 
                       apply(do.call(rbind, strsplit(as.character(history$trip_start_time[i[1]]), ":")), 
                             1, function(x) as.numeric(x[1]) + as.numeric(x[2]) / 60)
                   f
               })
    }, silent = TRUE)
    res <- sapply(fns, function(f) {
                      optimize(function(x) (f(x) - distance)^2,
                               interval = c(0, attr(f, "max")))$minimum
                  })
    info <- do.call(rbind, strsplit(names(res), ":"))
    out <- data.frame(date = info[, 1], trip_id = info[, 2], vehicle_id = info[, 3],
                      start_time = sapply(fns, function(x) attr(x, "start.time")),
                      start_hour = sapply(fns, function(x) attr(x, "start.hour")),
                      arrival_time = round(res))
    rownames(out) <- NULL
    
    out
}
histTravelTime <- function(origin, destination, history) {
    at_origin <- histArrivalTime(origin, history)
    at_destination <- histArrivalTime(destination, history)
    
    out <- data.frame(at_origin[, 1:5], 
                      origin = at_origin$arrival_time, 
                      destination = at_destination$arrival_time)
    out$travel_time <- with(out, destination - origin)
    o <- order(out$date, out$start_hour)
    
    out[o, ]
}
plotTravelTimes <- function(origin, destination, history) {
    T <- histTravelTime(origin, destination, history = history)
    iNZightPlots::iNZightPlot(y = travel_time, x = start_hour, boxplot = FALSE,
                              ylab = "Travel Time (s)", xlab = "Trip Start Time",
                              colby = date, pch = 19, cex.pt = 0.1, col.fun = colorspace::rainbow_hcl,
                              bg = "#252525", join = TRUE, data = T,
                              main = paste0("Time to travel from ", origin, "m to ", destination, "m"))
}
plotTravelTimes(0, 1000, HIST)
plotTravelTimes(0, 10000, HIST)
plotTravelTimes(0, 25000, HIST)
plotTravelTimes(5000, 10000, HIST)
plotTravelTimes(10000, 15000, HIST)
plotTravelTimes(20000, 21000, HIST)
@ 
