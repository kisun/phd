...
\hlstd{a[}\hlnum{1}\hlstd{, ]} \hlkwb{<-} \hlstd{a1}
\hlkwa{for} \hlstd{(i} \hlkwa{in} \hlnum{2}\hlopt{:}\hlnum{15}\hlstd{) \{}
    \hlstd{Delta} \hlkwb{<-} \hlkwd{arrivalTime}\hlstd{(stops[i])[,} \hlnum{1}\hlstd{]} \hlopt{-} \hlstd{ti}
    \hlstd{a[i, ]} \hlkwb{<-} \hlstd{ti} \hlopt{+} \hlkwd{rtnorm}\hlstd{(M,} \hlkwd{mean}\hlstd{(Delta),} \hlkwd{sd}\hlstd{(Delta)}\hlopt{/}\hlkwd{sqrt}\hlstd{(}\hlkwd{length}\hlstd{(Delta)),} \hlkwc{lower} \hlstd{= ti)}
\hlstd{\}}
...
\hlstd{predict} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{i}\hlstd{,} \hlkwc{draw} \hlstd{=} \hlnum{TRUE}\hlstd{,} \hlkwc{...}\hlstd{) \{}
    ...
    \hlstd{a1j} \hlkwb{<-} \hlkwd{arrivalTime}\hlstd{(stops[}\hlnum{1}\hlstd{])[,}\hlnum{1}\hlstd{]} \hlopt{-} \hlstd{ti}
    \hlstd{a1} \hlkwb{<-} \hlstd{ti} \hlopt{+} \hlkwd{rtnorm}\hlstd{(M,} \hlkwd{mean}\hlstd{(a1j),} \hlkwd{sd}\hlstd{(a1j)} \hlopt{/} \hlkwd{sqrt}\hlstd{(}\hlkwd{length}\hlstd{(Delta)),} \hlkwc{lower} \hlstd{=} \hlnum{0}\hlstd{)}

    \hlstd{a[}\hlnum{1}\hlstd{, ]} \hlkwb{<-} \hlstd{a1}
    \hlkwa{for} \hlstd{(j} \hlkwa{in} \hlnum{2}\hlopt{:}\hlkwd{length}\hlstd{(stops)) \{}
        \hlstd{Delta} \hlkwb{<-} \hlkwd{arrivalTime}\hlstd{(stops[j])[,} \hlnum{1}\hlstd{]} \hlopt{-} \hlstd{ti}
        \hlstd{a[j, ]} \hlkwb{<-} \hlstd{ti} \hlopt{+} \hlkwd{rtnorm}\hlstd{(M,} \hlkwd{mean}\hlstd{(Delta),} \hlkwd{sd}\hlstd{(Delta)}\hlopt{/}\hlkwd{sqrt}\hlstd{(}\hlkwd{length}\hlstd{(Delta)),} \hlkwc{lower} \hlstd{=} \hlnum{0}\hlstd{)}
    \hlstd{\}}
    \hlkwa{if} \hlstd{(draw) \{}
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/future_observations-1} 

\includegraphics[width=\maxwidth]{figure/future_observations-2} 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{28}\hlopt{:}\hlkwd{nrow}\hlstd{(test.trip))} \hlkwd{predict}\hlstd{(i,} \hlkwc{draw} \hlstd{=} \hlnum{FALSE}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning in optimize(function(x) (f(x) - d)\textasciicircum{}2, interval = range(hist\$time\_into\_trip)): NA/Inf replaced by maximum positive value}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `[<-`(`*tmp*`, j, , value = c(4723.99988213222, 4723.99988213222, : subscript out of bounds}}\end{kframe}
\end{knitrout}
