...
set.seed(345); par(mfrow = c(2, 1))
stops <- c(0, 100, 150, 200, 400, 450, 900, 1000) * 5
times <- c(0, 2, 8, 10, 25, 30, 50, 60) * 60
...
t <- 1:25 * 120
z <- sort(c(90, 110, 130, 140, 170, 200, 210, 230, 250, 270, 290, 320,
       350, 390, 410, 430, 460, 490, 520, 550, 580, 620, 650, 700, 750) * 5 + rnorm(length(t), 0, 30))
plot(times, stops, xlab = "Time (s)", ylab = "Distance Into Trip (m)", pch = 19, col = "red", cex = 0.5, type = "b",
     xlim = c(0, max(times)), ylim = c(0, max(stops)))
points(t, z)
...
plot(times, stops, xlab = "Time (s)", ylab = "Distance Into Trip (m)", pch = 19, col = "red", cex = 0.5, type = "b",
     xlim = c(0, max(times)), ylim = c(0, max(stops)))
polygon(c(t, rev(t)), c(qnorm(0.025, XX[, 1], SD), rev(qnorm(0.975, XX[, 1], SD))), lty = 0, col = "grey")
lines(t, XX[, 1])
