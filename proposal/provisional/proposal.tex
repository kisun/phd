...
% While there are obvious advantages to using a fast algorithm, 
% there are also disadvantages that are the result of many assumptions 
% and simplifications made in order for it to be so.
% In the case of the \kf{}, there are two main issues we are concerned with.
% The first of these is the lack of flexibility in the transition and measurement functions,
% which are restricted to matrices.
% It is therefore difficult to model complicated behaviours (see \cref{sec:bus-behaviour}),
% and also, in the case of observations from a \gls{gps} device,
% the relationship between observation state and true state cannot be expressed in $\mat{H}$.
% \cite{cathey-dailey:2003} proposed an algorithm which first estimated the distance into trip
% of a bus before using the estimate as the observation for the \kf{}.
