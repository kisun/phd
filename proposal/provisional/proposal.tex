...
One widely used type of \gls{avl} is the \gls{gps},
which is used in \gls{gtfs}-realtime feeds and is implemented in the Auckland Transport
...
The simplest prediction techniques use a combination of previous arrival travel times.
...
However, this method requires an \gls{avl} system that records
...
Recursive Bayesian filters are a class of model first used in bus prediction in 
(YEAR) by \cite{cn},
...
but assumed to be a part of a Markov process.
That is, the state at time $k$ is dependent only on the previous state, $k-1$;
i.e., $p(X_k | X_0, X_1, \ldots, X_{k-1}) = p(X_k | X_{k-1})$.
...
Instead of modeling the state distribution entirely, the \gls{pf} uses a sample of independent 
...
In our case, we assume the bus starts with a state of $\bX_k = [d_k\ v_k]^T$,
and an individual particle is denoted $\bX_k^{(i)} = [d_k^{(i)}\ v_k^{(i)}]^T$.
...
\end{equation}
Since each particle has its own value of $d_k$, we can use the shape information supplied by \gls{gtfs} to
find the coordinates of that particle (see FIGURE).

[! FIGURE]

Now, instead of generating pseudo-observations of distance, we are mapping the unknown state into 
a space we can compare directly to the observations:
\begin{equation}
...
of all $M$ particles (note that the likelihood uses the distance between the points):
\begin{equation}
...
are made, and we are no longer restricted to linear transitions between states 
...
we have available, to obtain a sample of arrival times, 
...
Therefore, the only way to implement this is by increasing the covariance estimates.
...
So far, we have only been looking at modeling the current state of a vehicle in real-time.
We now need to see how this is then applied to predict arrival times at future stops along the route.
